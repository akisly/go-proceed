...
describe("the problem+json userAction decides what the screen does next", () => {
  it("never shows an in-progress label for an action nothing acts on", () => {
    // THE CORRECTION (source's final review, Important 4). These two used to
    // return "sending" and "awaiting_receipt" — labels that assert an
    // operation is under way — while `uploadCapture` set the state and
    // returned, with no retry of the PUT and no re-GET of the intent
    // anywhere in this client. "Потрібна дія" (the `failed` label) is the
    // truth: the client stopped and the next move is the foreman's. If
    // either of these ever legitimately returns an in-progress state again,
    // it must be because the code that performs that operation landed
    // alongside it.
