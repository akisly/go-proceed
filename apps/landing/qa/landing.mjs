...
// (DEV-025) at seven widths, checks for horizontal overflow and console errors,
...
// words (DEV-027 — a second browser with software WebGL for the particle dome),
...
// The four pages (DEV-025). Keep in step with `landingContent.pages`; the link
...
try {
  ...
  // DEV-026 — so only /product is asked.
  ...
  // EVERY INTERNAL LINK LANDS (DEV-025). On one page a dead anchor scrolled
  ...
  async function beamPixels() {
    ...
    await page.goto(`http://localhost:${PORT}/product`, { waitUntil: "networkidle0" }); // the board is /product's (DEV-026)
    ...
    const present = await page.evaluate(async () => {
      // `#board`: the ink pills carry a beam too since DEV-026, and the header's is first in the document.
  ...
  // NOTHING IN A FIRST FOLD STAYS HIDDEN (DEV-026). A `whileInView` entrance on
  ...
  // older than DEV-025, and filed under BL-059; the deep links are therefore
  ...
  async function parity() {
    ...
    // (the home hero's until DEV-026). Bring the view in, read, scroll on, read.
    ...
    // PRODUCT — the sticky list marks the step in view (DEV-026: the
    ...
    // [DEV-027, seventh pass; 2026-09-22, owner: «хедер всегда сделай таким типа прозрачным, а не только на скрол»]
  ...
  //
  // [DEV-026] The counts below follow the reference's form: the board and its
  // three satellites are on /product; every ink pill is a magnetic control.
  // [DEV-025] One tilted surface on the whole site — the board. The
  ...
  /**
   * DEV-027 — THE POINTER-REACTIVE CANVAS WORDS, measured, because nothing else
