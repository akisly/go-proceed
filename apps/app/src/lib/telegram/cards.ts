...
export function formatAssignmentCard(payload: AssignmentCardPayload): TelegramFormattedMessage {
  ...
  const fixedLength = prefix.length + requirementsHeading.length + instruction.length;
  let remaining = MAX_TELEGRAM_MESSAGE_CHARACTERS - fixedLength;
  const title = fitEscaped(payload.title, remaining);
  remaining -= title.length;

  const lines: string[] = [];
  for (const [index, occurrence] of payload.occurrences.entries()) {
    const label = `${index + 1}. `;
    const separator = " — ";
    const suffix = index === payload.occurrences.length - 1 ? "" : "\n";
    const minimum = label.length + separator.length + suffix.length;
    if (remaining <= minimum) break;
    const criterion = fitEscaped(occurrence.criterion, remaining - minimum);
    remaining -= label.length + criterion.length + separator.length;
    const normRef = fitEscaped(occurrence.normRef, Math.max(0, remaining - suffix.length));
    remaining -= normRef.length + suffix.length;
    lines.push(`${label}${criterion}${separator}${normRef}${suffix}`);
  }

  return {
    text: `${prefix}${title}${requirementsHeading}${lines.join("")}${instruction}`,
    parseMode: "HTML",
  };
}
