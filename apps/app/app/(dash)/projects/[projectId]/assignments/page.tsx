...
/**
 * `/projects/{projectId}/assignments`'s own index — Plan D slice D1,
 * Task 5. THIN, per `docs/design/03-ui-references.md` §"The hierarchy": wires
 * the one route param this screen needs, calls the services it needs (two
 * since DEV-035: the register, and the project's name for its frame), and
 ...
 * (e.g. from a future link on `/`) re-renders only this page segment, not
...
type AssignmentsPageProps = {
  ...
  // `searchParams`, and `app/(app)/a/[assignmentId]/page.tsx`'s [deleted 2026-09-23, DEV-035] identical use
  // for `params`.
...
export default async function ProjectAssignmentsPage({ params }: AssignmentsPageProps) {
  ...
  // DEV-035: the project's name, for the frame's heading and breadcrumb.
  const [result, projectName] = await Promise.all([
    listAssignments(projectId),
    getProjectName(projectId),
  ]);
  if (result.kind === "session_expired") {
    redirect(`/login?next=${encodeURIComponent(`/projects/${projectId}/assignments`)}`);
  }
  ...
  if (result.assignments.length === 0) return <NoAssignmentsEmptyState projectId={projectId} projectName={projectName} />;
  return <AssignmentsList assignments={result.assignments} projectId={projectId} projectName={projectName} />;
}
