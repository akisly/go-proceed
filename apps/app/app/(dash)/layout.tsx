 ...
 *
 * [2026-09-23, DEV-035 — HISTORY BELOW, KEPT AS WRITTEN. The owner retired
 * the field PWA (`app/(app)/**`) whose root page caused the collision this
 * paragraph describes, and the dashboard moved to `app/(dash)/**` — the route
 * group it could not use then — serving `/**`; `/dash/**` redirects
 * (`next.config.ts`).]
 *
 ...
 * (`v1`, `external`, `_next`, a handful of static files) — `/` was never
...
export default async function DashRouteLayout({ children }: { children: ReactNode }) {
  ...
  // endpoint is caught at the one place that gates the whole `/**`
  // tree — the same reasoning `app/(app)/page.tsx` [deleted 2026-09-23, DEV-035] documents for its own two
  // catches.
  const projectsResult = await listProjects();
  if (projectsResult.kind === "session_expired") redirect(`/login?next=${encodeURIComponent("/")}`);
  ...
  // reads the `.kind`.
  //
  // `next=/` IS COARSER THAN THE PAGE-LEVEL REDIRECTS BELOW IT, and that
  ...
  // deep link to `/settings/profile` that expires between the proxy and
  // this render comes back to `/`. Every COLD open of that URL is still
  ...
  if (identityResult.kind === "session_expired") redirect(`/login?next=${encodeURIComponent("/")}`);
