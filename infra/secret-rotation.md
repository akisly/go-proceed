...
3. **Before trusting the database again, compare it with the migrations.** No list of queries is complete, so do both:
   ...
   - **A first look by query**, all read-only, as `postgres`:
     ...
     - grants to the exposed and application roles: `select grantee, table_schema, table_name, privilege_type from information_schema.role_table_grants where grantee in ('anon', 'authenticated', 'goproceed_app', 'goproceed_service');` and the same from `information_schema.routine_privileges`;
