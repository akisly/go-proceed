...
| Local development | `apps/*/.env.local` (git-ignored, `.gitignore:15`) and the local Supabase stack | Loopback-only role passwords `app_pw` / `service_pw` / `purge_pw` from `scripts/set-local-app-password.mjs`, which refuses any non-loopback host; the published local Supabase demo keys; the `dev1` HMAC keys in `apps/app/.env.example` |
...
| `goproceed_purge_worker_login` password, inside `PURGE_DB_URL` (DEV-036) | The five `app.*upload*purge*` functions: expiring intents and marking purges, across every workspace; not the bytes themselves, which need `SUPABASE_SECRET_KEY` | Replace in place, a third value; a purge run in flight fails and the next run retries (the one-hour claim window) |
| `CRON_SECRET` (DEV-036) | Triggering `GET /internal/evidence/purge` (idempotent; deletes only what is already due) | Replace in place in Vercel and redeploy at once. Whether Vercel sends the project's new value to the still-serving old deployment before the redeploy is not documented (unverified); a run refused with 401 in between is harmless, since the purge is idempotent and the next run catches up |
