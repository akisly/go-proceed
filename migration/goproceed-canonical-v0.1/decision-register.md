...
| D-045 | 2026-07-30 | The v0.1 technical catalogs define 70 design relations (66 tables plus 4 rebuildable projections), above the spec's 50–53 estimate; every addition (serialization heads, allocation/valuation ledgers, decision coverage, claim-scope lineages, review target sets, source-amount resolutions) is required verbatim by the approved domain/architecture documents. | Table count is an outcome, not a constraint; each extra relation carries a distinct identity, lifecycle, constraint, or retention need named in `technical/database/entity-catalog.csv`. | `technical/database/entity-catalog.csv` |
| D-046 | 2026-07-30 | The spec §12 logical name `review_decisions` is implemented as `internal_review_decisions`. | Remove permanent ambiguity with the two external decision tables while preserving the spec's logical intent; recorded here instead of silently renaming. | `technical/database/entity-catalog.csv` |
| D-047 | 2026-07-30 | The v0.1 API scope lists 51 operations, above the plan's 35–45 estimate. | Workflow completeness: invitation acceptance, source-amount resolutions, upload receipt re-fetch, and the external shell/exchange pair are explicit rows rather than being folded into neighbours; count is an outcome, not a constraint. | `technical/openapi/scope-v0.1.csv` |
| D-048 | 2026-07-30 | pnpm build scripts are allowlisted to esbuild and sharp only; puppeteer's Chrome download stays an explicit step (`pnpm exec puppeteer browsers install chrome`), as CI already does. | Supply-chain: install scripts run only for two vetted native packages; the browser download is auditable and cache-friendly as an explicit command. | `package.json` |
