...
| **`validate_package.py` crashes on absent hard-read docs** | `scripts/validate_package.py:756` reads `docs/20-flow-catalog.md` unconditionally via `.read_text()`; ten further sites in the same file read other docs the same unconditional way (e.g. lines 248–254, 758, 1725, 2303). Removing or renaming any one of those documents produces an unhandled `FileNotFoundError` traceback instead of a clean, reportable validation failure. Deferred because Child A does not require changing any of those documents, and a proper fix means wrapping every hard read with an existence check (or catching `FileNotFoundError` and converting it into a `require(False, ...)` failure) consistently across all ~11 sites — a general validator error-handling refactor, not a Child A concern. **Deferred repository technical debt** — do not expand Child A into that refactor |
...
| **A.4.19 cannot be satisfied as written without changing `prototype/`** | A.4.19 requires `make validate` to exit 0 **and** `git diff --exit-code -- prototype` to exit 0. Measured on 2026-07-26 against an untouched checkout: `make validate` exits 0 but modifies 14 tracked files (`prototype/qa-results.json` plus 13 of the 19 `prototype/qa-screenshots/*.png`) — the direct consequence of the row above, since `validate-qa` runs `prototype/qa/verify.mjs`, which regenerates those exact files, and `scripts/validate_package.py:2435-2447` then requires them to exist and match `qa-results.json`. The two halves of A.4.19 are therefore mutually exclusive under the current harness, and changing `prototype/` to fix it is out of scope for Child A (see "Any modification to `prototype/`" above). `.github/workflows/ci.yml`'s `package-validate` job enforces only the first half (`make validate` exits 0) and documents inline why it deliberately does not assert the second. Two options exist to close this gap later, neither attempted here: (1) make `prototype/qa/verify.mjs` write its screenshots and `qa-results.json` to a gitignored directory instead of tracked paths, mirroring `apps/demo`'s approach (A.3.2b); or (2) stop tracking those generated artifacts in git and drop the corresponding hard assertions from `scripts/validate_package.py`. **Deferred repository technical debt — a known, evidenced deviation from A.4.19's literal text, not an oversight** |
