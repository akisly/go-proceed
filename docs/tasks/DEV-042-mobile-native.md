...
- State: blocked (merged in #115 and deployed 2026-09-23; database suites PASS 2026-09-24 (row 13); waiting on devices and store accounts, the Android native build, and the owner decisions below)
...
| 13 | Coordinator | **Database suites run (owner consent, 2026-09-24).** `pnpm --filter @goproceed/testing test` (runs `supabase db reset` itself): 55 files / 827 tests PASS, none skipped; the INV-108 probes and the reference-image RLS cases ran (confirmed by a verbose re-run of `m1-project-sourced-schema` and `m1-rules-rls`, 55/55). `pnpm --filter @goproceed/app test` with CI's local credential env: `reference-images.int.test.ts` failed in fixture setup — the test never created its two `auth.users` rows (memberships FK); fixed in this change, both cases then PASS. A second full run was disturbed by a parallel session in the same checkout (main moved to #118 with 0096–0097 not in the reset database; hook timeouts); 0096–0097 hand-applied locally as `postgres` in single transactions; clean full run at `d07c9136`: 130 files, 1442 PASS, 2 FAIL — both `deploy-preflight-keys` cases, stale since DEV-036 and fixed on main by #119 (DEV-045); re-run on this branch (based on `20f2b674`): 58/58. `m3-refusal` failed in the first two full runs (one extra `work_stage.closed` outbox row) and passed alone (29/29) and in the clean run — recorded as BL-145. | test logs in the session scratchpad | Android native build, devices, store installs, owner decisions |
...
| Stale/partial xcframework or tarball reused | reviewer m7, security m5 | Fixed: `scripts/sodium-pin.json`; versioned build dirs; atomic extract; stamp-checked, staged xcframework. Independent minisign verification of the digest: open *[2026-09-24, DEV-056: closed — every libsodium build verifies the author's minisign signature ([DEV-056](DEV-056-sodium-minisign.md))]* |
...
| Stale `authenticate` reopening the native identity; minisign digest verification | security | **Owner acceptance owed** for the internal beta *[2026-09-24, DEV-056: the minisign half is built, not accepted — the owner chose verification at build time; the stale `authenticate` half is unchanged]* |
...
| Database suites (`apps/app` integration, `packages/testing`, including the INV-108 probes) | Yes | full `apps/app` run at `d07c9136` (before #119); fix `0644ada4` on `20f2b674` (reference-images suite and the preflight test re-run there) | `packages/testing` 55 files / 827 PASS; `apps/app` 1442 PASS, 2 FAIL outside DEV-042 (fixed by #119); row 13 | PASS | Local stack; CI not run (Actions billing) |
...
- *[2026-09-24]* Database suites: **PASS** (row 13). Still NOT RUN: an authenticated device pass, the Android native vault build, the device matrix, signing and TestFlight/Play Internal. State stays **blocked** on those and the owner decisions.
