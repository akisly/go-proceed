...
|---|---|---|---|---|---|
| U5 | minor | `Checkbox.tsx` | the 16px box sat 4px (desk) / 14px (touch) in from the column edge | Coordinator | Fixed (first as `-mx-1 touch:-mx-3.5`, then start-only after U6); §6: painted left edge equals the column edge at 1440 and 390 (DEV-073 row 4) |
| U6 | minor | `Checkbox.tsx` (the U5 fix) | cancelling the END inset let the 2px + 2px focus ring reach the label text (touch, and case 13 at the desk) | Coordinator | Fixed: start-side margin only (`-ms-1 touch:-ms-3.5`, logical per the re-check); measured at 1920/1440/1240/768/390/360 on both kitchen-sink checkboxes: painted left = column left, ring clearance to the label's first glyph 4px (case 13) and 8px (case 14), hit 24×24 desk / 44×44 touch, checked fill `rgb(12, 12, 10)` (`shots3/measure-final.json`) |
