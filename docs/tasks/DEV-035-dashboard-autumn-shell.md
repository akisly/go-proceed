...
- State: verifying — `gp-qa` round 2 PASS on all thirteen criteria (2026-09-23, on the revision with sha256 `d72c5dc2…`; the Q2 remarks were applied after it as prose/test-label/positive-control edits). Committed, pushed and opened as a PR to `main` on 2026-09-23 at the owner's word («сделай коммит, пуш и PR в main»); still the owner's: the NOT RUN items, the «Before merge» checks, and the merge
...
| 8a · database suites (`apps/app/tests/*.int.test.ts`, `packages/testing` resetDb suites) | yes | working tree on `27dea79`, diff sha256 `30b1f218c9a856f7…` | — | NOT RUN | environmental: they truncate or reset the shared local stack and the owner has not consented; settles with `pnpm turbo run test --concurrency=1` on a disposable stack |
| 8b · CI | yes | working tree on `27dea79`, diff sha256 `30b1f218c9a856f7…` | — | NOT RUN | not-provable-locally: GitHub Actions billing block |
