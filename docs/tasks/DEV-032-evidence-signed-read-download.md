...
| R1-04 | minor | evidence revision | Actual: the pre-rebase revision | coordinator | Runs re-done on the rework (row 6) |
...
- **Evidence finalized before this change is not re-checked**: an object already available with another stored type keeps it.
