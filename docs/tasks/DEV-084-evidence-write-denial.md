...
- Required acceptance criteria:
  ...
  - AC-2: each of these mutations fails a test, except where the mutant cannot be observed across workspaces, which is stated:
    ...
    - `ui_insert`'s capability inverted (inverting `ce_insert`'s would fail both controls trivially), and the project scope dropped.
...
| R1 | low | The afterAll | = S2 | Coordinator | Fixed |
| R2 | low | DA-114 | `superseded` is no status the catalog defines | Coordinator | Fixed: `normative`, with a dated note |
| R3 | nit | `scopeAnswer` | The refusal checked by code alone | Coordinator | Fixed: by its message |
