# DEV-052 — BL-144: the schema list and two review fixes get their tests; the cluster's final run

## Assignment

- **Objective and user-visible outcome:** no behaviour changes. `m1-schema.test.ts` lists `project_responsibility_assignment_ends` with the other workspace-access tables; `project_responsibilities.end`'s lower-casing of the member id and `revokeProjectAccessRequest`'s bound on `capabilities` get tests. Then the cluster «Доступ к проекту» (DEV-046 to DEV-053) gets its final run: the suites that drive the changed routes and policies, one at a time, plus types and docs.
- **State:** verifying
...
- **Linked spec, ADR or earlier task:** BL-144, filed by [DEV-044](DEV-044-responsibility-end.md); cluster DEV-046 to DEV-053.
- **Baseline:** `6fae61c0` (DEV-051).
...
- **Required acceptance criteria:**
  ...
  4. The cluster's final run: every suite named in DEV-046 to DEV-053's criteria passes on the final revision, one at a time, with none skipped; `tsc` and `validate:canonical-docs` pass.
...
|---|---|---|
| 2026-09-24 | BL-144 joins the cluster | chat, answer «… BL-144 тесты» (DEV-046's record) |
| 2026-09-24 | No local reset; truncating tenant tables is allowed | the session's standing brief (DEV-046's record) |
