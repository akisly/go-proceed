...
| Q2-01 | low (record only) | This record, the Q1-01 bullet's last sentence | Expected: the summary holds for every gap it follows. Actual: «no tracked file holds a line citation in any of those forms», while the exempt DEV-003 to DEV-006 records hold 52 lines of them | coordinator | The sentence now limits itself to unscanned extensions and the unmatched forms, and names the exempt records' counts |
...
**Rework round 1** (after the narrow QA re-check did not verify Q1-01's fix). Q2-01: a sentence summarising three disclosed gaps was checked against two of them and not against the exemption it followed. Changed practice: a summary claim is re-run against each case it summarises, the exempt ones included. One of three rounds used.
