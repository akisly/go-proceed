...
| [DEV-060](DEV-060-no-product-temporary-schema.md) | verifying | BL-155 (P2): no product role holds TEMPORARY on the database, so no product session has a temporary schema (migration `0102`, INV-116); the Supabase-managed roles keep it by direct grant; BL-157 files the ACL-drift gap |
| [DEV-066](DEV-066-validate-package-deleted.md) | implementing | BL-063: the orphaned `scripts/validate_package.py` is deleted (owner); what it asserted and what it found on its last possible run are recorded |
| [DEV-067](DEV-067-typescript-6.md) | implementing | BL-062: one TypeScript, 6.0.3, in the whole workspace; packages that use Node APIs declare `@types/node` |
| [DEV-068](DEV-068-lockfile-react-guard.md) | implementing | BL-083: one `@types/react`/`@types/react-dom` via overrides, `apps/mobile` `react-dom` 19.2.3, and a CI lockfile check |
| [DEV-069](DEV-069-vitest-5.md) | implementing | BL-061: vitest 3.2.4 → 5.0.1 with Vite 8.0.13; `vitest.workspace.ts` → `test.projects` |
