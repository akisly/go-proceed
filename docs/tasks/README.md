...
| [DEV-059](DEV-059-temporary-schema-searched-last.md) | verifying | BL-152 (P1): every definer and path-pinning function lists `pg_temp` last (migration `0101`, INV-115); the definer rule in `agents/COMMON.md` changes from «empty path» to «`pg_temp` last»; BL-155 files PUBLIC's TEMP |
