...
| [DEV-046](DEV-046-access-helpers-search-path.md) | implementing | BL-143: the three SECURITY DEFINER workspace-access helpers pin an empty search path (migration `0098`) |
| [DEV-047](DEV-047-project-path-ids.md) | implementing | BL-141: a malformed path id on any `/v1/projects/{projectId}` route is 404 before the key, the body and the database; the error catalog's `retryable` is documented as a default |
| [DEV-048](DEV-048-project-view-window.md) | implementing | BL-140: a grant keeps the member's `project.view` covering their unexpired action capabilities — a lapsed or shorter view is replaced, a short view-only grant is 422 |
