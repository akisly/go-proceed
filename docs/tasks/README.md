...
| [DEV-047](DEV-047-access-helpers-search-path.md) | done | BL-143: the three SECURITY DEFINER workspace-access helpers pin an empty search path (migration `0098`) |
| [DEV-048](DEV-048-project-path-ids.md) | done | BL-141: a malformed path id on any `/v1/projects/{projectId}` route is 404 before the key, the body and the database; the error catalog's `retryable` is documented as a default |
| [DEV-049](DEV-049-project-view-window.md) | done | BL-140: a grant keeps the member's `project.view` covering their unexpired action capabilities — a lapsed or shorter view is replaced, a short view-only grant is 422 |
| [DEV-050](DEV-050-removal-reports-remaining.md) | done | BL-142: removing a member from a project reports what stays live — their active review links on it (no address) and whether a Telegram group is connected; nothing cascades (ADR-014 decision 5) |
| [DEV-051](DEV-051-last-admin-records.md) | done | BL-137: the records say the product already keeps an undated administrator on every project; what can still orphan one is SQL, a future suspension (BL-014) or an administrator who left; BL-137 to P3 |
| [DEV-052](DEV-052-grant-revoke-write-once.md) | done | BL-138: a grant accepts only its revoke, once, at the transaction's time, and is never deleted — for every role (migration `0099`, INV-113); fixtures stage lapsed or removed grants in replica mode |
| [DEV-053](DEV-053-review-follow-up-tests.md) | done | BL-144: `m1-schema` lists the end table, and the end route's lower-casing and the revoke request's capability bound get tests; the cluster's final run |
| [DEV-054](DEV-054-window-ends-after-start.md) | done | BL-149: a grant or assignment whose `validUntil` does not come after its start is 422 on `validUntil` before the database, not 500 |
| [DEV-055](DEV-055-inlined-helpers-qualified.md) | done | BL-150, BL-106: the three SQL helpers inlined into definers and policies name `pg_catalog.uuid` and `pg_catalog.current_setting` (migration `0100`, INV-114); BL-152 files the bare casts in definer bodies |
| [DEV-056](DEV-056-sodium-minisign.md) | done | Every libsodium build verifies the author's minisign signature (key pinned in `minisign.mjs`, the release's `.minisig` in the pin) beside the sha256 digest; closes DEV-042's open minisign item |
| [DEV-057](DEV-057-pwa-retirement-loose-ends.md) | done | BL-135: `apps/mobile` headers say which copies are canonical and which are deliberate duplicates, the duplicated user-facing text is held equal by tests, dead web icons and the unused `safe-next.ts` are deleted; `/a/{id}` stays 404 (owner) |
| [DEV-058](DEV-058-field-project-sourced-disclaimer.md) | done | BL-154: the field obligation list prints the project-sourced items disclaimer, only when an item is labelled «за робочою документацією об'єкта», immediately after the довідковий one; both mobile copies are held byte-equal to the content rules |
| [DEV-059](DEV-059-temporary-schema-searched-last.md) | done | BL-152 (P1): every definer and path-pinning function lists `pg_temp` last (migration `0101`, INV-115); the definer rule in `agents/COMMON.md` changes from «empty path» to «`pg_temp` last»; BL-155 files PUBLIC's TEMP |
| [DEV-060](DEV-060-no-product-temporary-schema.md) | done | BL-155 (P2): no product role holds TEMPORARY on the database, so no product session has a temporary schema (migration `0102`, INV-116); the Supabase-managed roles keep it by direct grant; BL-157 files the ACL-drift gap |
| [DEV-061](DEV-061-field-client-decisions.md) | done | The field client's open owner decisions: offline sign-out, a wipe for a vault that cannot open, reset after reinstall, a held discard, the requirement on the queue card, bounded discard waits |
| [DEV-063](DEV-063-app-qa-otp-diagnostics.md) | done | BL-158: the daylight audit's code request records what GoTrue answered and what the page showed on each failed attempt, and repeats once, leaving a `::warning::` annotation on a pass that needed it; the cause stays open |
| [DEV-066](DEV-066-validate-package-deleted.md) | done | BL-063: the orphaned `scripts/validate_package.py` is deleted (owner); what it asserted and what it found on its last possible run are recorded |
...
| [DEV-070](DEV-070-ios-upload-stream.md) | done | iOS photo uploads no longer crash the app: the vault's upload body is a bound stream pair fed by a producer thread (CFNetwork's HTTP/2 path cannot drive an `InputStream` subclass); the body completes only after the reader's authenticated EOF, and any producer failure cancels the task |
| [DEV-071](DEV-071-database-acl-compared.md) | done | BL-157 (P3): INV-116's catalog checks live in one read-only SQL file that the test suite, `pnpm db:catalog-snapshot` (exit 1 on a violation) and the hosted comparison in `infra/README-staging.md` §2.3 all run; first hosted run on `goproceed-staging`: 0 rows; BL-161 files the missing restore procedure |
| [DEV-073](DEV-073-stock-tailwind-under-tokens.md) | done | BL-047: stock Tailwind stays whole and the tokens ride on top (ADR-015); `max-w-md` compiles again, so dialogs, the login column and the empty states take their width |
| [DEV-074](DEV-074-checkbox-hit-area.md) | done | BL-048: `Checkbox`'s hit area is 24px at the desk and 44px on touch around a 16px box |
| [DEV-075](DEV-075-requirement-list-disclaimers.md) | implementing | BL-156: the Telegram card, the requirement-choice prompt and the office's blocked-reasons list print the довідковий disclaimer and, when an item is project-sourced, the project-sourced note; the texts have one source in `apps/app` |
