...
| [DEV-054](DEV-054-window-ends-after-start.md) | verifying | BL-148: a grant or assignment whose `validUntil` does not come after its start is 422 on `validUntil` before the database, not 500 |
| [DEV-055](DEV-055-inlined-helpers-qualified.md) | verifying | BL-149, BL-106: the three SQL helpers inlined into definers and policies name `pg_catalog.uuid` and `pg_catalog.current_setting` (migration `0100`, INV-114); BL-151 files the bare casts in definer bodies |
