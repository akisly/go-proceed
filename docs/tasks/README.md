...
| [DEV-063](DEV-063-app-qa-otp-diagnostics.md) | verifying | BL-158: the daylight audit's code request records what GoTrue answered and what the page showed on each failed attempt, and repeats once, leaving a `::warning::` annotation on a pass that needed it; the cause stays open |
| [DEV-066](DEV-066-validate-package-deleted.md) | verifying | BL-063: the orphaned `scripts/validate_package.py` is deleted (owner); what it asserted and what it found on its last possible run are recorded |
| [DEV-067](DEV-067-typescript-6.md) | verifying | BL-062: one TypeScript, 6.0.3, in the whole workspace; packages that use Node APIs declare `@types/node` |
| [DEV-068](DEV-068-lockfile-react-guard.md) | verifying | BL-083: one `@types/react`/`@types/react-dom` via overrides, `apps/mobile` `react-dom` 19.2.3, and a CI lockfile check |
| [DEV-069](DEV-069-vitest-5.md) | verifying | BL-061: vitest 3.2.4 → 5.0.1 with Vite 8.0.13; `vitest.workspace.ts` → `test.projects` |
