...
| [DEV-073](DEV-073-stock-tailwind-under-tokens.md) | verifying | BL-047: stock Tailwind stays whole and the tokens ride on top (ADR-015); `max-w-md` compiles again, so dialogs, the login column and the empty states take their width |
| [DEV-074](DEV-074-checkbox-hit-area.md) | verifying | BL-048: `Checkbox`'s hit area is 24px at the desk and 44px on touch around a 16px box |
