...
| [DEV-034](DEV-034-storage-errors-without-keys.md) | verifying | BL-033: every error `evidence-storage.ts` throws is an `EvidenceStorageError` naming the operation and the provider's code — never the storage key, the bucket or the provider's message |
