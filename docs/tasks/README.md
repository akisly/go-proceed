...
| [DEV-056](DEV-056-sodium-minisign.md) | reviewing | Every libsodium build verifies the author's minisign signature (key pinned in `minisign.mjs`, the release's `.minisig` in the pin) beside the sha256 digest; closes DEV-042's open minisign item |
| [DEV-057](DEV-057-pwa-retirement-loose-ends.md) | reviewing | BL-135: `apps/mobile` headers say which copies are canonical and which are deliberate duplicates, the duplicated user-facing text is held equal by tests, dead web icons and the unused `safe-next.ts` are deleted; `/a/{id}` stays 404 (owner) |
