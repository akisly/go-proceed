...
| [DEV-031](DEV-031-outputs-guards.md) | verifying | BL-081: `.gitignore` ignores everything under `outputs/` but its pointer README, and the validator refuses a tracked ProZorro `contactPoint` in a dump's forms, any other tracked `outputs/` file and any tracked spreadsheet; its two `outputs/` exemptions removed |
