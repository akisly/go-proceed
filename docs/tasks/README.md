...
| [DEV-032](DEV-032-evidence-signed-read-download.md) | implementing | BL-089: every member-plane evidence signed read is a download (`Content-Disposition: attachment`), so opening one never renders the uploader's content type on the Storage origin; the evidence card's `<img>` is unchanged |
