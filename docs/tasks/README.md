...
| [DEV-043](DEV-043-project-access-revoke.md) | implementing | BL-021: a project administrator revokes a member's project grants by member and capability; revoking `project.view` removes the member from the project; the last live administrator grant is kept (409 `PROJECT_FINAL_ADMIN`); the application role may update only `revoked_at` and `version` (ADR-014, migration `0096`) |
| [DEV-044](DEV-044-responsibility-end.md) | implementing | BL-015: a project administrator ends a member's responsibility on a project at the moment of the command, by an append-only end fact per assignment; a later assign no longer counts it in its warnings (ADR-014, migration `0097`) |
