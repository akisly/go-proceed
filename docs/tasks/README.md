...
| [DEV-030](DEV-030-outputs-private-storage.md) | verifying | BL-079: the prospecting session's 250 files, which hold personal data, leave the tree for a private folder outside the repository, copied from git with a verified SHA-256 manifest; `outputs/README.md` becomes a pointer with no personal data; `bbfc705` still holds them (no history rewrite) |
| [DEV-031](DEV-031-outputs-guards.md) | implementing | BL-081: `.gitignore` ignores everything under `outputs/` but its pointer README, and the validator refuses a tracked ProZorro `contactPoint` in a dump's forms, any other tracked `outputs/` file and any tracked spreadsheet; its two `outputs/` exemptions removed |
