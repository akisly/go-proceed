...
| [DEV-047](DEV-047-access-helpers-search-path.md) | verifying | BL-143: the three SECURITY DEFINER workspace-access helpers pin an empty search path (migration `0098`) |
| [DEV-048](DEV-048-project-path-ids.md) | verifying | BL-141: a malformed path id on any `/v1/projects/{projectId}` route is 404 before the key, the body and the database; the error catalog's `retryable` is documented as a default |
| [DEV-049](DEV-049-project-view-window.md) | verifying | BL-140: a grant keeps the member's `project.view` covering their unexpired action capabilities — a lapsed or shorter view is replaced, a short view-only grant is 422 |
| [DEV-050](DEV-050-removal-reports-remaining.md) | verifying | BL-142: removing a member from a project reports what stays live — their active review links on it (no address) and whether a Telegram group is connected; nothing cascades (ADR-014 decision 5) |
| [DEV-051](DEV-051-last-admin-records.md) | verifying | BL-137: the records say the product already keeps an undated administrator on every project; what can still orphan one is SQL, a future suspension (BL-014) or an administrator who left; BL-137 to P3 |
| [DEV-052](DEV-052-grant-revoke-write-once.md) | verifying | BL-138: a grant accepts only its revoke, once, at the transaction's time, and is never deleted — for every role (migration `0099`, INV-113); fixtures stage lapsed or removed grants in replica mode |
| [DEV-053](DEV-053-review-follow-up-tests.md) | verifying | BL-144: `m1-schema` lists the end table, and the end route's lower-casing and the revoke request's capability bound get tests; the cluster's final run |
| [DEV-054](DEV-054-window-ends-after-start.md) | verifying | BL-149: a grant or assignment whose `validUntil` does not come after its start is 422 on `validUntil` before the database, not 500 |
| [DEV-055](DEV-055-inlined-helpers-qualified.md) | verifying | BL-150, BL-106: the three SQL helpers inlined into definers and policies name `pg_catalog.uuid` and `pg_catalog.current_setting` (migration `0100`, INV-114); BL-152 files the bare casts in definer bodies |
