...
| Q1 | minor | `xlsx-guard.ts`, `ignoreBOM` | A BOM-prefixed `xl/macros/…` name, refused at the baseline, passed | Coordinator | Fixed: U+FEFF refused; test |
...
- Next bounded action and owner: `gp-reviewer` on the validator change and the Q fixes; CI.
