...
| 12 | rework (coordinator), rework round 2 | Q2-01 and Q2-02 applied as QA's smallest fixes, in this record only | See Findings | `gp-qa` narrow re-check of the record |
...
- **Rework round 2** (after QA round 2 FAIL). Q2-01 and Q2-02 are bookkeeping misses in this record: R2-02's relabel covered the location cell but not the same row's text, and Progress was not advanced when the re-review ran. Changed hypothesis: after each stage, the coordinator adds its Progress row before applying its fixes, and a relabel is searched across the whole record. Two of three rounds used. One of three rounds used.
...
| 5. Independent `gp-qa` on the final revision | yes | `d4a3860` | `gp-qa` round 1 on `d4a3860`: needs fixes (Q1-01, Q1-02). Round 2 on `5036d04`: needs fixes (Q2-01, Q2-02, record only) | NOT RUN | Narrow re-check of the record pending |
