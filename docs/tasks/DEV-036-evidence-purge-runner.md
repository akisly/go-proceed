...
| R2-01 | minor | `evidence-purge.ts` `finish()` (with DEV-038) | Actual: `err.name` is "error" for every database error | coordinator | SQLSTATE logged; asserted (row 8) |
...
- Final state and reason: done, merged as #112 (`dc24922`) — every required criterion PASS on `37b7f11` and the closing fix; CI not required.
