...
| Q1 | minor | `tw-merge.test.ts:16` | the test built its own `cn`, so reverting `cn.ts` to `override` passed every test | Coordinator | Fixed: imports the real `cn`; mutation now fails 2 of 9 (row 6) |
...
| AC-6 full CI run | Yes | `3fc05553` | PR #144 CI run 36045655102, `verify` + `app-qa` green (row 6) | PASS | |
