# DEV-023 — The landing rebuilt 1:1 against its reference, in our colours

## Assignment

- Objective and user-visible outcome: the four landing pages of DEV-022 take the visual treatment of https://parlo-black.vercel.app/ — page frame, full-viewport hero, section bands, two-tone headings, pill controls, sticky feature list, grid cards, fact band, FAQ split, radial closing block, and their animations — rendered in the Daylight colours (`DESIGN.md`) on paper, not on black. Closes BL-082.
- State: verifying
...
- Read context and applicable local instructions: as DEV-022, plus `docs/design/02-building-ui.md` §7.3 (a new motion primitive is a decision).
- Linked spec (`docs/specs/…`), ADR or earlier task: DEV-022 (the four pages, the structure this task dresses); DEV-007 and BL-082 (the reference decision and its open questions).
- Baseline: the DEV-022 working tree, snapshot tree `0edd98ad8fb2602a155c2df2e9bb6e172dbecd9e` over commit `1f65fef` (DEV-022 is uncommitted; the snapshot is a `git write-tree` from a scratch index, the real index untouched).
...
- Required acceptance criteria:
  ...
  3. Each reference composition listed in «Mapping» as built is present on the page the mapping names, and DEV-022's acceptance criteria 1, 3, 4, 5 and 6 still hold (its criterion 2, the home page's block list, is replaced by this task's).
...
|---|---|---|
| 2026-09-19 | «я же попросил что бы лендинг выглядел 1 в 1 как https://parlo-black.vercel.app/ с анимациями карточками переходами только что бы цвета сохранились наши: ну и подход к страницам тоже наш» | Owner, this session, after seeing DEV-022's home page |
...
|---|---|---|---|---|
| 0 | coordinator | DEV-022 had taken only block *types* from the reference; the owner, shown its home page, said that was not the request (see «Owner decisions»). DEV-022's pages, metadata, form placement and tests are the base; its block compositions are replaced here. | DEV-022 record | — |
...
| R-02 | major | `gp-reviewer` · `scenes.tsx` | One `Stagger` round three stacked cards: DEV-022's U-01 again, and its guard test had been deleted. | coordinator | Fixed: one `Reveal` per card; guard re-homed («reveals each card on its own»). |
