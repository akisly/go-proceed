...
| 7 | gp-reviewer | R1 PASS: R1-01 low (pre-existing: `app.current_actor()`'s unqualified `::uuid`), R1-02..R1-05 low (STATUS, DA-198, evidence, criterion 2), R1-06 nit (BL-145) | reviewer report, 2026-09-24 | fixes |
...
|---|---|---|---|---|---|
| R1-01 / S1-03 | low | `app.current_actor()` (`0003`) inlined into the helpers | its `::uuid` is an unqualified type a session's `pg_temp` can shadow; pre-existing | coordinator | filed BL-149 (P2); `0098`'s comment names the exception; the owner is asked whether to fix it now |
...
- `app.current_actor()`'s unqualified `::uuid` is still shadowable inside the helpers (BL-149).
