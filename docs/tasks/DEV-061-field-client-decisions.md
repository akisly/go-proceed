# DEV-061 — The field client's open owner decisions
...
| 11 | gp-qa; Coordinator | gp-qa on `e86975bb`: every Fixed finding in place (file:line), typecheck and 217/217, validators, motion-audit clean, the S-02 mutation check reproduced (both guards removed → the two race tests fail; either alone is covered by the other); **FAIL**: main had taken DEV-058, BL-155 and INV-115 meanwhile. Fixed: `origin/main` `919a8c10` merged, renumbered to DEV-061, BL-157, INV-116/117. The post-wipe login was reshot on the final JavaScript (Android debug build, JS from Metro): «Фото на пристрої стерто.» shown as a neutral notice after «Стерти фото на пристрої». Owner observation: signed in on the iOS simulator's debug build and saw «Не вдалося завантажити ваші доручення» — Metro had been stopped, and the debug build loads the lazily imported `./supabase` (`api.ts` `requireSession`, which predates this task) from Metro; production logged no request from the app. With Metro restarted and the app reloaded, both staging assignments listed. | gp-qa report (session); `a16-final-after-wipe.png`; Vercel runtime logs (production, 2 h: only 401s from the coordinator's probes); simulator screenshot | gp-qa re-check |
| 12 | gp-qa (re-check); Owner; Coordinator | gp-qa: the collision FAIL resolved; merge-tree clean; 225/225 (main's `lib/field` tests account for the extra eight). main's DEV-059 record announces DEV-060 for the BL-155 cluster, so this task took DEV-061. Owner chose a signed-in pass on the iOS simulator now. It began: the assignment list, the requirement screen and a gallery import («Фото збережено на пристрої») worked against staging. The automatic send then **crashed the app**: `NSInvalidArgumentException: -setDelegate: only defined for abstract class` from the vault's `VaultInputStream` (an `InputStream` subclass used as the upload body) under CFNetwork's HTTP/2 request body stream. This is DEV-042 code, not this task's. Every iOS photo upload crashes, and a pending photo re-sends at each launch. The owner chose a separate task for the fix; the rest of the signed-in pass waits for it. | simulator log (`log show`, 17:51:54); screenshots | PR; the iOS upload fix |
| 13 | Coordinator (signed-in pass); Owner | On 2026-09-24, with the owner signed in to staging. Build: local branch `test/dev061-with-070` (this branch `155baa98` merged with DEV-070's `eb932d7e`, which fixes the iOS upload crash this pass had found), not pushed. iOS 18.1 simulator and the Android emulator; staging read-only SQL for server state.<br>• **Update is not a reinstall (iOS):** installing the build over the previous one kept the session and the vault.<br>• **Capture and send through the UI:** a gallery pick on the active assignment was saved and sent; intent `5ed4241a…` (1,896,240 bytes) `available`.<br>• **Queue card label:** a capture carrying the requirement label (153 characters, the assignment's acceptance criterion verbatim) shows it on its card cut to two lines; a tap expands it to the full text (screenshot `dev061-queue-label-expanded.png`).<br>• **Discard hold:** discarding capture `a94a0e31…` (DEV-070's test B leftover: imported over the debugger at 18:23 on the DEV-070 build, ciphertext deliberately corrupted; the discard ran on this build), whose intent `97c6a8cf…` could still be received, showed «Фото буде видалено пізніше»; the card became «Буде видалено», «Надіслати зараз» disappeared and the header counts ↑ 0. Natively `discardRequestedAt` was set; `upload` and `markAwaitingReceipt` refused with `VAULT_DISCARD_REQUESTED`. A pull-to-send created no new intent.<br>• **Immediate discard without an intent:** a capture the server refused before any intent was removed at once, without a hold.<br>• **Profile, held item:** «Ненадісланих фото немає. Видалені фото (1) буде прибрано…» (`dev061-profile-held.png`).<br>• **Sign-out with a held item (iOS, online):** «Вийти» led to the login screen; the journal row became `quarantined` and kept `discardRequestedAt`. After the owner signed in and opened the assignment, it returned as `not_sent` with the mark kept, still shown «Буде видалено»; staging still had one intent for it, `intent_authorized`.<br>• **Reinstall reset (iOS):** with the owner signed in, `simctl uninstall` then `install`: the app opened on the login screen although the Keychain survives an uninstall (control key, row 5; not re-checked here); `GoProceedInstallation` was written anew (`dev061-reinstall-login.png`).<br>• **Offline sign-out (Android):** the owner signed in; airplane mode on (ping: network unreachable); «Вийти» showed the offline confirmation («Немає з’єднання: сервер не дізнається про вихід…», `dev061-android-offline-confirm.png`), and confirming it opened the login screen. After a force-stop and relaunch still offline, and 20 s after airplane mode was turned off, the app stayed on the login screen (the stored session was gone; the force-stop ended the process, so the in-process late-refresh latch was not exercised here — the unit tests cover it).<br>Not run in this pass: a hold resolved against staging (the held intent expires 24 h after creation, and the reinstall removed the held capture), the received-anyway notice, and «Стираємо…» with a real session. | simulator and emulator screenshots (scratchpad), debugger `list`, journal read with `sqlite3 mode=ro`, staging SQL (read-only) | gp-qa |
| 14 | gp-qa | Row 13 checked claim by claim against the code on `155baa98` and the screenshots; the test build carries only DEV-070's `NativeVault.swift` change on top of this branch (no JavaScript, no Android). Split signed-in row: PASS after wording fixes (the held capture's origin, the late-refresh latch not exercised by a force-stop, the Keychain claim not re-checked, «Android emulator», two stale sentences); the narrowed signed-in row and S-08: NOT RUN, stated honestly. `pnpm validate:canonical-docs` OK. Fixes applied. | gp-qa handoff | Owner: the remaining NOT RUN rows |
| 15 | Owner; Coordinator | Merged in [#135](https://github.com/akisly/go-proceed/pull/135) as `28b83be0` on 2026-09-24. CI on its last revision: `verify` and `app-qa` passed. The four unrun criteria stay with BL-160; the S-03 residual with BL-159. | GitHub PR #135 | Done |
...
- **Catalogs and docs:** transition rows, INV-053, INV-117, INV-118, the ADR-013 amendment and the spec correction.
...
| R9 | minor | tests, INV-118 witness | tests that can fail; witnesses named | Coordinator | Fixed: the timeout test releases the run after the hold; R1 tests; INV-118 names the device pass and marks the error-state choice unwitnessed |
| R10, R11, R12 | nit | INV-117/118, ADR, transition row | wording matches the mechanism | Coordinator | Fixed |
...
- **Development builds only.** A React warning («Can't perform a React state update on a component that hasn't mounted yet») appears in development builds; it predates this task and was not investigated. A debug build whose Metro is stopped cannot load lazily imported modules (`api.ts` `requireSession` imports `./supabase` dynamically) and shows «Не вдалося завантажити ваші доручення» without sending a request; release builds bundle everything (row 11). The owner's sign-in listed both staging assignments once Metro was back: partial signed-in evidence; the signed-in pass is row 13.
...
- Review independence: independent — `gp-mobile`, `gp-architect`; `gp-reviewer` and `gp-security` two rounds each; `gp-ui-reviewer` three rounds; `gp-qa` (rows 11, 12, 14). All subagents.
- Verified scope: unit tests, native builds, the vault on the Android emulator and iOS simulator, iOS reinstall behaviour, the vault-error UI, and the signed-in pass against staging (row 13).
- Remaining risks / blocked requirements: see What is not true; four criteria deferred by the owner to BL-160; the S-03 residual in BL-159; no physical device (BL-002).
- Next bounded action and owner: none in this task; BL-160 before field phones are used in a pilot.
