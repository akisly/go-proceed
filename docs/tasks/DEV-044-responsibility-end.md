...
- **Required acceptance criteria:**
  ...
  6. The existing suites that drive the assign route or read the schema pass — the coordinator's set: `projects.int.test.ts`, and in `packages/testing` `m1-schema.test.ts`, `rls-coverage.test.ts`, `workspace-access-rls.test.ts` (non-resetting files, run by path).
