...
| 11 | coordinator | Round-2 fixes: the view is written with the request's string when its end is the request's own; the response names each granted row's `validUntil` (owner, S1-01); the audit's `view` gains `replacedValidFrom`; tests for the replaced future view, the `need = required` repair, the sub-millisecond end (red with the fix removed) and the disclosure; INV-111 reworded; the block moved above DEV-050's comment. Grant suite 15 passed | `scratchpad/dev048-review-green-r2.txt`, `dev048-r2-01-mutation.txt` | gp-qa |
...
| S1-04 | info | INV-111; this record | rows written before DEV-048 can still be in the BL-140 state until the next grant to that member | coordinator | INV-111 «Not covered» and «What is not true» amended |
...
| R2-05 | info | the grant suite | the block sat under DEV-050's comment | coordinator | fixed |
...
- Grant rows written before DEV-048 can still leave an action without a covering view until the next grant to that member; the Telegram resolver (0084) checks `evidence.record` alone (gp-security S1-04).
