...
- Remaining risks / blocked requirements: BL-158's cause is still unknown. Four app-qa runs so far include the change. In three of them the harness ran and passed on the first attempt. The fourth died in `supabase start`. The BL-158 number also collides with open PR #135, which renumbers when it lands. The 2026-10-31 fallback in BL-158 still needs the owner's agreement.
