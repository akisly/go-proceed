...
| Telegram channel (ADR-011, M7) | Merged and enabled in no environment: #58 (2026-09-03, `0061`–`0081`, including #62's `0080` and #65's identity erasure `0081`, both merged into its branch), #68 and #69 (2026-09-04, `0082`–`0083`), #78 (the card's tag and source, `0084`, 2026-09-13) | Merge commits; `apps/app/vercel.json` has no crons, so `app/internal/telegram/jobs/route.ts` has no caller; `src/lib/telegram/ingress.ts` has no rate-limit code | Before any environment enables the webhook: the Task 13 edge rate limit, a scheduler for the jobs route (Q-12), and the real-group staging pass (Q-17, blocked by M0's real-data rule). Retention durations are NULL (`TODOS.md:3313`) |
