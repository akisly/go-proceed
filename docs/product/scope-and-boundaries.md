...
*[2026-09-23, DEV-042 — changed by [ADR-013](../decisions/ADR-013-native-field-client.md), approved by the owner on 2026-09-22;
this note records that change and does not rewrite the scope wording above.
The field client is the native iOS/Android build of `apps/mobile` (phones and
tablets, email OTP, the same bearer `/v1` API), not a web export: the owner
deleted the Vercel project `goproceed-field` and removed `FIELD_CLIENT_ORIGINS`
from `goproceed-app` on 2026-09-23, so the CORS layer is inactive. Durable
encrypted pending captures move into the v0.1 internal beta (next paragraph).
The native client is on branch `codex/mobile-native` (PR #115, not merged);
its device and store evidence is NOT RUN ([DEV-042](../tasks/DEV-042-mobile-native.md)). The Telegram channel is
unchanged.]*
...
for the product.
*[2026-09-23, DEV-042 — changed by [ADR-013](../decisions/ADR-013-native-field-client.md), approved by the owner on 2026-09-22;
the scope wording above is not rewritten. The native `apps/mobile` client
arrives in v0.1, not v0.3, and there is no web export any more (the owner
deleted `goproceed-field` on 2026-09-23). v0.1's connection-loss protection
becomes an encrypted foreground queue that survives a restart, with online
authorization; the five offline items listed above stay v0.3. Not merged;
device evidence NOT RUN ([DEV-042](../tasks/DEV-042-mobile-native.md)).]*
