...
| Vercel `goproceed-field`, root `apps/mobile`; CORS measured on the wire — `OPTIONS /v1/projects` echoes the exact origin, and a 401 still carries the `access-control-allow-origin` header *[2026-09-23, DEV-042: the project is deleted and `FIELD_CLIENT_ORIGINS` removed by the owner ([ADR-013](../decisions/ADR-013-native-field-client.md)); this measurement is history]* | same, :505-508, :703-716 | 2026-08-21 |
...
| Whether the Expo-web field client reaches parity | The parity gate is a measurement on two physical phones ([ADR-009](../decisions/ADR-009-three-pilot-surfaces.md):113-128), and **three of its five sub-items are unmeasured** (§8.3). The repository disagrees with itself on the hardware: [2026-08-01-b0-procurement.md](../superpowers/plans/evidence/2026-08-01-b0-procurement.md) and [TODOS.md](../../TODOS.md) record both devices as unprocured, while [README-staging.md](../../infra/README-staging.md):922-926 and :941-949 carry iPhone measurements dated 2026-08-21. **No Android measurement exists anywhere.** C-13. ADR-011 open item 8 adds one fact: the field client **stays available on a Telegram-locked project**, so the parity gate's meaning is unchanged by the channel *[2026-09-23, DEV-042: the Expo-web client is retired and [ADR-013](../decisions/ADR-013-native-field-client.md) (owner, 2026-09-22) supersedes the parity gate; the native client's gate is the physical device matrix of [DEV-042](../tasks/DEV-042-mobile-native.md), NOT RUN. The hardware disagreement above still stands.]* |
...
| C-8 | six files named at [ADR-009](../decisions/ADR-009-three-pilot-surfaces.md):154 | `glossary.md`, `roadmap.md`, `vision-and-positioning.md`, `personas-and-workflows.md`, `test-strategy.md`, `execution-and-evidence.md` still describe `apps/mobile` as v0.3 | Plan C landing *[2026-09-23, DEV-042: Plan C's Expo-web client is superseded by [ADR-013](../decisions/ADR-013-native-field-client.md)'s native client; see the note under §2 P2]* |
...
*[2026-09-23, DEV-042 — P2's «Plan C … field-client parity» is superseded by
[ADR-013](../decisions/ADR-013-native-field-client.md) (owner, 2026-09-22): the field client is the native iOS/Android build, and
its gate is the device matrix and store install of [DEV-042](../tasks/DEV-042-mobile-native.md); see the note under
§2 P2.]*
...
| **Blocks** | **Nothing in P1, P1b, P3 or P4a** — this is why the diagram above does not draw P2 into the M6 junction. Plan C blocks the retirement of the `apps/app` PWA field pages *[2026-09-23, DEV-035 — no longer: the owner retired them before the gate; see [ADR-009](../decisions/ADR-009-three-pilot-surfaces.md) «Amendment, 2026-09-23»]*. **But «and nothing else» would be false**, and the exception is operability rather than milestone closure: pilot *recovery* has no product route. [TODOS.md](../../TODOS.md) (now [BL-021](../BACKLOG.md#bl-021)) records that a project access grant issued through the product **cannot be revoked through it** — «The only route back is a superuser UPDATE» — and §6.1 records that `external_grants.revoke_reissue`, the only recovery INV-044 leaves for a lost link, reaches no browser at all. (Assignment *creation* is no longer in this list: D3's create screen landed in PR #54, which retires [2026-08-21-plan-d-dashboard.md](../superpowers/plans/2026-08-21-plan-d-dashboard.md):58's «today this is SQL».) Whether the revocation gap is accepted for the pilot or is a P2 exit condition is **undefined in repo**; §8.7 carries it as an operating risk rather than pretending it is closed. |
| **Standing rule** | «`apps/app`'s field pages stay deployed and functional and are the pilot's only working field client until Plan C's parity measurement lands. **No task in any of the three plans above may remove them first.**» ([TODOS.md](../../TODOS.md), now [BL-001](../BACKLOG.md#bl-001); [ADR-009](../decisions/ADR-009-three-pilot-surfaces.md):115-128) *[2026-09-23, DEV-035 — superseded by the owner: «Удалить сейчас» removed `apps/app`'s field pages before the parity measurement; see [ADR-009](../decisions/ADR-009-three-pilot-surfaces.md) «Amendment, 2026-09-23». The pilot's field client is now the Telegram project channel (not yet enabled in any environment, BL-024) and the `apps/mobile` Expo web export.]* |

*[2026-09-23, DEV-042 — covers the Plan C rows of both tables above. [ADR-013](../decisions/ADR-013-native-field-client.md)
(owner, 2026-09-22) supersedes ADR-009's Expo-web deployment and its parity
gate: the owner deleted the third Vercel project, `goproceed-field`, and removed
`FIELD_CLIENT_ORIGINS` from `goproceed-app` on 2026-09-23, so Plan B's CORS
layer is inactive. Plan C's client is now the native iOS/Android build of
`apps/mobile`; its entry and exit are ADR-013's physical device matrix and a
TestFlight / Play Internal install, NOT RUN in [DEV-042](../tasks/DEV-042-mobile-native.md). Plans B, C and D above are
the record of their dates.]*
...
**Update, 2026-09-17** ([DEV-014](../tasks/DEV-014-gate11-workspace-communication.md)). The workspace-access and communication gap rows (BL-098, BL-090) are covered: 50 `covered`, 24 `gap` (BL-091 to BL-097) and 7 `exempt_no_grant`. The gate stays open. BL-096's two service rows wait on BL-100, a projection policy that admits every workspace to the service plane.

**Update, 2026-09-17** ([DEV-015](../tasks/DEV-015-projection-service-policy.md)). Migration `0086` closes BL-100, and BL-096's two service rows are covered: 52 `covered`, 22 `gap` (BL-091 to BL-095, BL-097), 7 `exempt_no_grant`. `0086` is applied to the local database only. The gate stays open.

**Update, 2026-09-17** ([DEV-016](../tasks/DEV-016-gate11-remaining-gaps.md)). 21 of the 22 remaining gap rows are covered: 73 `covered`, 1 `gap`, 7 `exempt_no_grant`. The last gap is `capture_events` for `goproceed_service` (BL-102, a migration first). The gate stays open.

**Update, 2026-09-18** ([DEV-017](../tasks/DEV-017-capture-event-service-workspace.md)). Migration `0087` closes BL-102, the last gap: **74 `covered`, 0 `gap`, 7 `exempt_no_grant`**. The gate is closable and not closed — it waits on the unfiltered `pnpm --filter @goproceed/testing test` evidence run, a `rls-coverage.test.ts` run at `0087`, the `version-0.1.md` §M0 entry and the owner. A hosted push must deploy the application build before `0087` (§10 Q-9): the finalize path has to declare the intent's workspace first, or every finalize fails with 42501. Afterwards the same holds in reverse: rolling the application back past that commit while `0087` is applied is an evidence-finalization outage (the success path fails too, because the capture event shares its transaction with `app.finalize_upload_intent`), so roll the migration back with it or roll forward. `0087` also requires the service connection to authenticate as a login that is a member of `goproceed_service` (what `SERVICE_DB_URL` names): any other login fails closed with 42501 on every finalize.
...
**Status: PARTIAL, recorded 2026-09-15** ([DEV-012](../tasks/DEV-012-m0-gate12-evidence.md)). The three built halves are exercised against the local database (upload create 23, finalize 19, purge 17, imports 13, domain import 48, all passed, none skipped; not in CI), and the owner accepted the magic-byte check, the allow-list and the limits **in place of a malware control** for the pilot — no file is scanned — with the risk, the departures (`files-and-storage.md`, ASVS-FILE-08) and five revisit triggers named in [production-readiness.md](production-readiness.md) §12, without an ADR (Q-10 answered for the pilot). **Not closable yet:** export neutralization waits on item 3; image dimension and decoding limits do not exist (BL-088); member-plane reads are served inline with the stored content type (BL-089). *[Added 2026-09-15 (DEV-012); the table above is the state before it.]* *[2026-09-23, DEV-032: finalization refuses a stored type other than the detected one, and member-plane reads are issued as downloads; BL-089 closed, BL-126 open. DEV-033: image size limits at finalization, which bound the declared size, not the decoding cost; BL-088 closed, BL-129, BL-131 and BL-132 open, and the gate closes with BL-129 and BL-132 fixed or owner-accepted.]*
...
| Retiring the `apps/app` PWA field pages | **DONE 2026-09-23, by owner decision, before the gate** (DEV-035) | The owner's «Удалить сейчас» ([ADR-009](../decisions/ADR-009-three-pilot-surfaces.md) «Amendment, 2026-09-23») | **S** | The parity gate now measures the `apps/mobile` client's readiness only. *[2026-09-23, DEV-035 — was: «**BLOCKED by construction** \| The closed parity gate \| **S** \| Nothing until the gate closes — and the standing rule forbids doing it first».]* |

*[2026-09-23, DEV-042 — this table measures the retired Expo-web client. [ADR-013](../decisions/ADR-013-native-field-client.md)
(owner, 2026-09-22) supersedes the parity gate; the owner deleted
`goproceed-field` on 2026-09-23. The open units now are the native client's:
the physical device matrix, the Android vault build, signing and a TestFlight
/ Play Internal install, NOT RUN in [DEV-042](../tasks/DEV-042-mobile-native.md).]*
...
| The scheduler — something runs `delivery.ts` and the purge worker | **OPEN for `delivery.ts`** (Q-12, Q-9); the purge worker's runner is decided and built (2026-09-23, [DEV-036](../tasks/DEV-036-evidence-purge-runner.md)) | A runner, its deployment record, and the monitoring decision of §5.8 | **M**, and it is a decision first | Decide «what runs it» before building it |
...
| 5 | **C-7, C-8** — the roadmap and the six ADR-009 files | **L** | The roadmap carries no content after 2026-08-08 at all; C-8 blocks Plan C landing *[2026-09-23, DEV-042: Plan C's Expo-web client is superseded by [ADR-013](../decisions/ADR-013-native-field-client.md)]* |
...
- **`pnpm --filter @goproceed/mobile qa` additionally requires `apps/app` to be
  ...
  *[2026-09-23, DEV-042 — that command and `qa/field-web.mjs` are deleted with
  the Expo web field client ([ADR-013](../decisions/ADR-013-native-field-client.md)); the link above no longer resolves, and the
  native client has no harness yet ([DEV-042](../tasks/DEV-042-mobile-native.md)).]*
...
**Orphaned — do not wire:** `python scripts/validate_package.py` *[2026-09-24, DEV-066: deleted]* (~2400 lines
...
- **The Plan C parity gate is NOT RUN (`environmental:` two physical phones) until two physical phones exist.** A passing headless harness and a passing laptop smoke test are evidence *toward* it, not a substitute for it ([BL-001](../BACKLOG.md#bl-001)). *[2026-09-23, DEV-042: [ADR-013](../decisions/ADR-013-native-field-client.md) supersedes that gate; the native client's device matrix (iPhone, Android phone, iPad, Android tablet) and TestFlight / Play Internal install are NOT RUN (`environmental:`) in [DEV-042](../tasks/DEV-042-mobile-native.md), and a simulator pass does not substitute for them.]*
...
The `apps/app` PWA is **retired**: on 2026-09-23 the owner removed its field
pages before this gate was measured ([ADR-009](../decisions/ADR-009-three-pilot-surfaces.md) «Amendment, 2026-09-23»). The
pilot's field client is the Telegram project channel — built and enabled in no
environment yet (BL-024) — and the `apps/mobile` Expo client, deployed as a web
export at `goproceed-field`: observed by the coordinator on 2026-09-23 at 08:32
UTC serving its Ukrainian sign-in page and manifest, with a cross-origin read of
`goproceed-app` `/v1/projects` passing CORS (401, unauthenticated as expected);
its deployed commit was not observed. This gate is now the measurement of that
client's readiness, no longer a condition for removing anything, and
...
*[2026-09-23, DEV-035 — was: «Until the gate is measured, **the `apps/app` PWA
is the pilot's only working field client and stays deployed**, and [ADR-007]'s
Costs 1 and 2 apply to the Expo-web build exactly as they applied to the PWA.»]*
*[2026-09-23, DEV-042 — [ADR-013](../decisions/ADR-013-native-field-client.md) (owner, 2026-09-22) supersedes ADR-009's
delayed-retirement (parity) gate, and the client it measured is gone: the
owner deleted `goproceed-field` and removed `FIELD_CLIENT_ORIGINS` from
`goproceed-app` on 2026-09-23. The field client is the native iOS/Android build
of `apps/mobile`, and its readiness gate is ADR-013's physical device matrix
(iPhone, Android phone, iPad, Android tablet) plus TestFlight / Play Internal
install, NOT RUN in [DEV-042](../tasks/DEV-042-mobile-native.md). The browser sub-items above describe the retired web
client. Decision 5's withdrawn claims stay withdrawn; the native client records
`native_camera` or `photo_picker` as client-supplied provenance, never as
sensor authenticity.]*
...
| 9 | Does the pilot's field client have a measured parity gate — or is the pilot running on the `apps/app` PWA, which stays deployed until it does? | [ADR-009](../decisions/ADR-009-three-pilot-surfaces.md):113-128 and «Amendment, 2026-09-23» | **Neither, since 2026-09-23.** The PWA is retired; the field client is the `apps/mobile` Expo web export, whose parity gate is unmeasured (§8.3), and the Telegram project channel, enabled in no environment yet (BL-024). *[2026-09-23, DEV-035 — was: «**PWA.** Legitimate; the pilot is never blocked on this».]* *[2026-09-23, DEV-042: still **Neither** — the web export is retired too ([ADR-013](../decisions/ADR-013-native-field-client.md)); the field client is the native build, whose device matrix and store install are NOT RUN ([DEV-042](../tasks/DEV-042-mobile-native.md)).]* |
| 10 | Does anything in the demo, the landing page, a screen or a sales sentence claim a payment-presentation refusal that does not exist? | [version-0.1.md](version-0.1.md):908-913 | Must be re-checked before the first customer sees anything. **Owner and procedure: the role §3.3 parks as «not yet activatable» owns «the landing page's claims», so on the day this step runs the owner runs it directly.** The check itself is: grep `apps/landing`, the six `page.tsx` under `apps/app/app/dash` *[2026-09-23, DEV-035: now under `apps/app/app/(dash)`, served from the root]*, and every outreach letter under `discovery/templates/` for the positioning sentence, and confirm each occurrence carries the explicit «payment-presentation eligibility is not in v0.1» statement §9.3 requires. **No script does this** — undefined in repo, and it is the one step of this procedure with no mechanical form |
...
| Reverting to a single deployed surface, or moving the field client back into `apps/app` **before the parity gate is measured** | [ADR-009](../decisions/ADR-009-three-pilot-surfaces.md):156-162 *[2026-09-23, DEV-042: [ADR-013](../decisions/ADR-013-native-field-client.md) is the superseding ADR for the field client; the office surfaces are unchanged]* |
...
| **Q-10** | **Is magic-byte content-type enforcement plus a bounded allow-list accepted AS the malware control for a pilot, or is an engine owed?** An ADR-shaped decision nobody has written. *[2026-09-15, [DEV-012](../tasks/DEV-012-m0-gate12-evidence.md): answered for the pilot — accepted in place of a malware control, no ADR, with the risk and revisit triggers in [production-readiness.md](production-readiness.md) §12.]* | **Owner decision** (ADR) | M0 item 12 |
...
| **Q-12** | **What does «monitored» mean for a one-workspace pilot** — and **what runs the consumer?** `delivery.ts` exists and nothing schedules it; the purge worker has no runner; ADR-011 decision 10 names «the scheduler» as a webhook-enable blocker without saying what it is (pg_cron, a Vercel cron, a worker). A dashboard closes nothing. *[2026-09-23, owner, [DEV-036](../tasks/DEV-036-evidence-purge-runner.md): answered for the evidence purge only — Vercel Cron, four daily expressions on the Hobby plan, alerting by the run's 500 in the Vercel Cron log. A four-times-a-day cron cannot serve Telegram delivery, so the consumer half and «monitored» stay open.]* | DevOps Automator / **owner decision** | M0 item 8; M7's operational status |
