...
**Status: PARTIAL, recorded 2026-09-15** ([DEV-012](../tasks/DEV-012-m0-gate12-evidence.md)). The three built halves are exercised against the local database (upload create 23, finalize 19, purge 17, imports 13, domain import 48, all passed, none skipped; not in CI), and the owner accepted the magic-byte check, the allow-list and the limits as the pilot's malware control with the risk named in [production-readiness.md](production-readiness.md) §12, without an ADR. **Not closable yet:** export neutralization waits on item 3, and image dimension and decoding limits do not exist (BL-088). *[Added 2026-09-15 (DEV-012); the table above is the state before it.]*
