...
| [BL-022](#bl-022) | P2 | closed → DEV-088 | A hand-typed zero-priced line and an imported one store different provenance |
...
| [BL-064](#bl-064) | P1 | closed → DEV-082 | vertical-m1 steps 7 and 8 went red once and never again |
...
| [BL-105](#bl-105) | P3 | closed → DEV-084 | A capture event's work assignment is bound by nothing, so a defective service transaction could name another workspace's assignment |
...
| [BL-171](#bl-171) | P1 | closed → DEV-085 | 3 external_review registry rows lack a cross-workspace write-denial test |
| [BL-172](#bl-172) | P1 | closed → DEV-086 | 3 operational registry rows lack a cross-workspace write-denial test |
| [BL-173](#bl-173) | P1 | closed → DEV-086 | 2 projection registry rows lack a cross-workspace write-denial test |
...
| [BL-182](#bl-182) | P1 | closed → DEV-085 | The external-session write branches on evidence decisions and their heads have no cross-workspace write-denial test |
...
- **Evidence:** before DEV-088, `apps/app/src/lib/manual-baseline.ts:253` `priceBasis: unitPrice === null ? null : pins.priceBasis,`.
- **Depends on:** nothing.
- **Deadline:** none recorded.
...
- **State:** closed → DEV-082
- **Legacy cite:** `TODOS.md` «vertical-m1 steps 7 and 8 went red once and would not do it again»
- **Why:** *[2026-09-25, DEV-082: fixed in #160 and closed. Root-caused: `parseXlsx` handed ExcelJS `Buffer.from(bytes).buffer` — Node's whole shared allocation pool (64 KiB on Node 24, the engine this repository pins, for any buffer under 32 KiB) — so the parser read the workbook among leftover bytes of other allocations and threw on a stray zip signature, which the parser reports as `XLSX_MALFORMED` and the validate route as `failed`. Reproduced at about 1% of parses on Node 24.21.0 and never on Node 22. Re-ranked by DEV-082 (P2 → P1): it fails real imports, and the parser read up to 64 KiB of process memory the upload guard never checked. Whether the 2026-08-10 red had the same cause depends on the Node version that run used, which was not recorded.]* a real unexplained red in one full serialized run on 2026-08-10. The hypothesis to test first is cross-package residue from `@goproceed/testing` running before `apps/app`.
- **Evidence:** none since; CI history was not searched for a recurrence. *[2026-09-24: it recurred. CI run 36024385064 (`claude/app-qa-otp-flake`, `9f885170`, verify job 107717069989): step 7 failed at `vertical-m1.int.test.ts:190` with «expected 'failed' to be 'preview_ready'», then step 8 failed at `:222` with «expected { …(11) } to deeply equal undefined». The change under test touched only `apps/app/qa/field.mjs` and docs. Recorded by [DEV-063](tasks/DEV-063-app-qa-otp-diagnostics.md).]* *[2026-09-25: it recurred on docs-only #157 (CI run 36131358586, attempt 1, verify job 108059118983): step 7's validate answered `failed`; the one re-run passed. Root-caused by [DEV-082](tasks/DEV-082-xlsx-parse-reads-the-buffer-pool.md).]*
- **Depends on:** a recurrence: capture the assertion text before re-running. *[Satisfied 2026-09-25: the #157 recurrence above.]*
- **Deadline:** none recorded.
...
- **Scope (DEV-086 gp-architect F2, 2026-09-25):** `idempotency_records` likewise: `goproceed_service` inherits its INSERT and `idem_insert`, which reads the actor and never the declared workspace, so an actor-bearing service transaction records a reply wherever the actor is an active member. Five routes write through such a transaction (the communication card, communications and their retry, and the Telegram member-link and binding intents). `packages/testing/src/operational-write-rls.test.ts` asserts the actor fence on the service plane, not a declaration fence.
...
- **State:** closed → DEV-086
- **Legacy cite:** none
- **Why:** *[2026-09-25, DEV-086: `idempotency_records` is `covered` by `packages/testing/src/operational-write-rls.test.ts` on the member plane and the service plane that inherits it; `0110` narrowed its INSERT to the eleven columns its writer writes (owner). Merged in #168 (`f75b4373`).]* *[2026-09-25, DEV-085: the `audit_events` and `transaction_outbox` rows are covered by `packages/testing/src/external-review-write-rls.test.ts` (member and external planes; the owner folded them in); `idempotency_records` stays open.]* BL-099, widened by the owner on 2026-09-24 («widen now, in stages»; DEV-076): a `covered` row of `technical/database/rls-coverage.csv` whose principal holds a write needs a cross-workspace write-denial test, and `technical/database/rls-write-coverage.csv` classifies these 3 relation–principal rows (3 relations) as `gap`: `public.audit_events` (goproceed_app): INSERT; `public.idempotency_records` (goproceed_app): INSERT; `public.transaction_outbox` (goproceed_app): INSERT. The minimum per row, set by DEV-076 with the owner on 2026-09-24 (`docs/delivery/test-strategy.md` §4): on the member plane, an active member of another workspace holding every capability the policy asks for; on the service plane, another declared workspace and none. For each privilege the row names: an INSERT carrying the other workspace's tenant key and parent ids refused by the policy (42501), with the same statement succeeding in the own workspace as the control, and an INSERT carrying the own tenant key with the other workspace's parent id refused by the policy (42501) or the composite foreign key (23503); an UPDATE and a DELETE that read no column — no `WHERE`, a constant `SET`, no `RETURNING`, since a `WHERE` would be answered by the read policy alone — run in a rolled-back transaction, succeeding with a row count equal to the own-workspace rows it may change (at least one; a statement that fails proves nothing about the policy), with the other workspace's rows read back unchanged as admin; and, where the principal can UPDATE the tenant key or a parent column, its own rows refused when moved into the other workspace by an UPDATE that likewise reads no column — with a `WHERE`, the SELECT policy applied to the new row refuses the move even under `WITH CHECK (true)` (DEV-077, observed on 17.6), so it would mask the policy under test. A trigger's refusal does not count: the assertion runs with `ALTER TABLE … DISABLE TRIGGER USER` (not `ALL`, and not `session_replication_role = replica`, which also switch off the foreign keys' own triggers), or an unused write grant is revoked by a migration instead. A write row may not cite its read row's own test (gp-security S5). A test that closes a row is cited in `technical/database/rls-write-coverage.csv`, which the validator and `rls-coverage.test.ts` then check. Ranked by DEV-076 (owner: P1).
...
- **State:** closed → DEV-085
- **Legacy cite:** none
- **Why:** *[2026-09-25: closed by DEV-085. `0109` added the session's workspace to the four external INSERT policies and `redh_external_update`'s WITH CHECK (the owner chose the policy term over accepted key-only confinement); `external-review-write-rls.test.ts` asserts the external decision and head branches; the external decision route names its head by workspace and occurrence (S2). Merged in #166 (`f3f994b2`).]*
  ...
  - **What the external policies do not read (gp-reviewer R2).** None of the three reads `workspace_id` or `project_id`. So on the session's own occurrence, a decision or head carrying B's tenant key, and a head moved to `workspace_id = B`, are admitted by the policy and refused only by a composite FK (`requirement_evidence_decisions_occurrence_fkey` / `…_session_scope_fkey`, `requirement_evidence_decision_heads_occurrence_fkey`). The test asserts those as 23503 with the FK named. Then either the FK-only confinement on this plane is accepted in writing, or a policy predicate (`workspace_id` equal to the session's workspace) is added (`gp-architect`, `gp-security`).
  - **The two other external insert branches.** `audit_insert_external` and `outbox_insert_external` are named as untested by `docs/delivery/test-strategy.md` §4, and belong in the same slice.
...
<a id="bl-183"></a>
### BL-183 — P3 — Three requirement UPDATE grants are whole-table where their routes set a few columns
...
<a id="bl-201"></a>
### BL-201 — P3 — A readiness projection's scope reference has no key, so a service write can name another workspace's scope
