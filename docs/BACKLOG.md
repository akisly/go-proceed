...
| [BL-148](#bl-148) | P3 | scheduled → DEV-054 | A grant or assignment whose `validUntil` does not come after its start answers 500, not 422 |
| [BL-149](#bl-149) | P2 | scheduled → DEV-055 | `app.current_actor()` casts to an unqualified `uuid`, which a session's temporary schema can shadow inside the definer helpers |
