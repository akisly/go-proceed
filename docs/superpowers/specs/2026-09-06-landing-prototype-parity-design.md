...
| 1g | Pulse | `.tag.rv::before` 1.6 s infinite, opacity .3↔1, scale .8↔1.1 (l.112–113) | none | `Chip pulse` on every `.tag.rv`: the board's two review cards and the Фіксація «пілот» chip. Not the receipt's state row (text, no dot) and not the route window's «на розгляді» tag — that one is `.tg.rv` (l.545), which the prototype does not pulse | — (CSS) |
