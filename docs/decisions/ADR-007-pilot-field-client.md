...
> [Amended 2026-09-03 by ADR-011 decision 11: read «never cited **as evidence**
> for anything». The authority for making Telegram the project's transport is
> the owner's instruction of 2026-08-28, recorded in ADR-011 — a capture-UX
> decision taken on instruction and before validation; `A-8` stays Unvalidated
> and the evidence that would move it is still owed.] `A-4` —
...
[Amended 2026-09-03 by ADR-011 decision 11. On the Telegram path the
constraint is satisfied by construction: capture **is** sending a photo to the
project's Telegram group, as a reply to an assignment card (ADR-011 decision
5). The sentence stays true there and stops being a comparison; on the PWA path
it remains the benchmark this decision was measured against.]
...
- the upload protocol
  ...
  `evidence_available` [2026-09-03: on the Telegram path the hash is computed
  by the server from the provider's bytes — ADR-011 decision 11];
...
- a **client-computed content hash**, verified at finalization against the bytes
  ...
  [Amended 2026-09-03 by ADR-011 decision 11: on the Telegram path a
  client-computed hash is not claimable — the server computes the hash from
  the provider's bytes (`apps/app/src/lib/telegram/evidence.ts`), so what may
  be claimed there is a server-computed content hash of the bytes as received,
  which likewise proves nothing about origin. The four withdrawn claims stay
  withdrawn on both paths];
