...
--
-- Additive. Withdraws grants. No policy, no column, no row.
--
...
--     app.consume_telegram_member_link_intent (0085);
...
-- or supabase/functions/ writes these tables or these columns outside tests,
...
-- 2026-09-25, DEV-078). Every SELECT grant stays.
--
...
begin
  ...
  if not has_table_privilege('goproceed_service', 'public.telegram_member_links', 'SELECT')
     or not has_any_column_privilege('goproceed_service', 'public.telegram_chat_bindings', 'UPDATE') then
