...
- **P3 — `ScrollTint`/`LineDraw`'s docstrings still say «only two scroll-linked primitives» / count two** (`packages/ui/src/motion/index.ts:8`, `ScrollTint.tsx`); there are three scroll-linked primitives in the vocabulary now (`ScrollSettle` joined them). Rule 9 is a per-page budget, not a vocabulary size, so the rule itself still holds — only the comment's count is stale.
