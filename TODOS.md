...
**Corrected 2026-08-29 — the count above undercounted by one; the 2026-08-28 measurement stays as written.** The gate reports THREE files, not two. `node packages/testing/qa/motion-audit.mjs` re-run today exits 1 with `motion-audit: 3 finding(s)`, adding `apps/landing/components/blocks/evidence-journey-client.tsx:9` to the two named above, and `npx vitest run src/motion-audit.test.ts` inside `packages/testing` fails `motion audit — the product > finds nothing` with all three names (12/13 in that file; the full-suite `625/626` figure was NOT re-run today and stands as measured). The third file was never outside the audit's reach — `ROOTS` has included `apps/landing` and `EXCLUDED` has been empty since `0fd6721` (2026-08-21), and the script is unchanged since — so the red set has been three since `bbfc705` landed on 2026-08-28. Documentation only: the «What» paragraph now names all three files, and the primitive rewrite it prescribes is still this entry's unfixed fix, now one file wider.
...
- **P3 (CLOSED 2026-09-06 — the sticky lives in `ScrollStackCard`; the literal media query is gone) — `landing-route-card` carries a literal `1240px` media query** in `apps/landing/app/globals.css` because `@variant` is unavailable inside a plain class rule at that layer. The one place the breakpoint is typed outside the token; fold it into a `base.css` utility when a second sticky pattern appears.
- **P3 — `FeatureCell`'s icon box is `size-(--gp-control-height-touch)`** (44px) in `packages/ui/src/components/FeatureGrid.tsx:59`, where the prototype's icon tile is 36px (`size-(--gp-control-height-desk)`). Cosmetic only — the touch floor is a safe default, not a defect — but worth a deliberate pick rather than the desktop cell inheriting the touch size.
- **P3 (CLOSED 2026-09-06 — rule 9 and the docstrings were rewritten by the parity slice) — `ScrollTint`/`LineDraw`'s docstrings still say «only two scroll-linked primitives» / count two** (`packages/ui/src/motion/index.ts:8`, `ScrollTint.tsx`); there are three scroll-linked primitives in the vocabulary now (`ScrollSettle` joined them). Rule 9 is a per-page budget, not a vocabulary size, so the rule itself still holds — only the comment's count is stale.
- **P3 — `channel-app.tsx`'s badge «пілот» is a literal** (`apps/landing/components/visuals/channel-app.tsx:12`) rather than sourced from `demoRecords`/content, unlike the sibling channel visuals. Low risk — it is demonstration chrome, not user-facing product copy — but it drifts silently if the pilot-state label ever changes elsewhere.
- **P3 — the pilot form's failed-card mailto uses raw fields while the clipboard text uses the cleaned ones** (`apps/landing/components/blocks/pilot-form.tsx`): `buildPilotMailto(fields)` reads the unvalidated `fields` state (set from the raw form read before `validatePilotFields` runs), while the clipboard copy on the same failure path uses `checked.fields` from `buildPilotClipboardText`. The two fallbacks can carry slightly different text (e.g. un-stripped control characters) for the same failed submission.
- **P3 — `ui-capture.tsx`'s phone-shot caption is a `span`** (`apps/landing/components/visuals/ui-capture.tsx`), so the `.landing-photo > figcaption` rule in `apps/landing/app/globals.css:133` never matches it; the caption currently inherits acceptable styling by accident, not by the rule that was written for it.
- **P3 — the pilot rate limiter's current-request protection is not guaranteed by content, only by insertion order** (`apps/landing/app/api/pilot/rate-limit.ts`): the bounded-map eviction's second pass deletes the oldest-inserted keys unconditionally once the first (expired-only) pass is not enough, and `Map.set` on an already-tracked key does not move it later in iteration order — so a returning caller's own key can be evicted by a burst of spoofed `x-forwarded-for` values even though it was just used. Worth a guard (skip the current key in the unconditional pass) or, short of that, a comment noting the gap explicitly.
- **P3 — `design-contract.test.tsx`'s `it()` title still says «ships the approved evidence-journey direction»** (`apps/landing/tests/design-contract.test.tsx:8`) though the contract id it asserts is `user-approved-daylight-2026-09-05`. The assertion is correct; only the test's own name is stale. [2026-09-06: the id is now `user-approved-daylight-parity-2026-09-06`; the stale `it()` title stands]

#### From the daylight visual pass (2026-09-05)
...
The whole-branch review of `claude/practical-chatterjee-c8d64a` (four areas,
0 Critical · 17 Important · 58 Minor) fixed all seventeen Importants under the
controller's rulings R1–R12, plus six Minors those rulings named. These are the
fifty-two Minors that were parked — one line each, so none is silently dropped —
and two of the reviewers' Recommendations worth the same treatment. All P3.

**Foundation — tokens, tests, brand pipeline**

- `packages/testing/src/palette-derivation.test.ts:8-20,72-78` — the header still says «no hex in the source was typed by a human»; under Daylight most steps are the prototype's picked hexes re-encoded as OKLCH. The test is sound, the prose overstates.
- `packages/testing/src/contrast.test.ts:74-75` — `text-accent`'s «large text only» is documented, not enforced; nothing fails if it lands on a 16px paragraph. A `motion-audit`-style source check would make the ruling real.
- `packages/testing/src/palette-derivation.test.ts:96-112` — only the two ends of the neutral ramp are pinned; the warm→cool hue crossover between `-300` and `-400` is the structural fact and is asserted nowhere.
- `apps/app/public/icon.svg`, `apps/app/public/safari-pinned-tab.svg` — generated by `scripts/generate-brand-icons.mjs` (`copyFileSync`) with no GENERATED header, so CLAUDE.md rule 2 cannot protect them. A byte-identity test against the brand sources is three lines.
- `package.json:27` — `sharp` is pinned to `0.34.5` while `next@16.3.1` already resolves `sharp@0.35.3`; the lock carries two sharp trees and two `@img/sharp-libvips-*` sets. Pinning `0.35.3` dedupes for free.
- `scripts/generate-brand-icons.mjs` — no `scripts` entry; every other repo script has one. `"brand:icons": "node scripts/generate-brand-icons.mjs"`.
- No drift check on the icon outputs: a brand SVG can be edited without the PNGs following, unlike `tokens generate`, which is gated on a clean `git status`.
- `apps/mobile/assets/android-icon-background.png` is generated and never read — `app.json` uses `adaptiveIcon.backgroundColor`, not `backgroundImage`. Drop the file or use it.
- `green-500`, `amber-500` and `cobalt-50` are reachable from no semantic role, yet spec §4.1 names `green-500 #1E8F5A` as «ok (icons, dots)» — the spec promises a colour nothing can legally use. Add the role or drop the promise.

**`packages/ui`**

- `packages/ui/src/base.css:200` — `@utility beam` writes only `-webkit-mask`, unlike `marquee-mask` and `spotlight`; on an engine that does not alias it the element paints the whole conic gradient over the frame instead of a 1px ring.
- `packages/ui/src/base.css:207` — `gp-beam 7s` is a literal where `marquee-track` reads `var(--gp-duration-marquee)`; a `--gp-duration-beam` token would make «two passes over 14 seconds» a reviewable value.
- `packages/ui/src/components/FeatureGrid.tsx:60-63` — `FeatureCell` fixes its heading at `h3` and folds the subtitle inside it, so every outline entry is two sentences. `BentoCell` and `Step` are likewise hard-`h3`; a `headingLevel` prop is what makes them reusable.
- `packages/ui/src/components/Stepper.tsx:17-27,46-52` — a four-step plan has no list semantics: a `<div>` of bare `<article>`s, with order and total reaching AT only through the visible `when` labels.
- `packages/ui/src/components/Compare.tsx:29-32` — `CompareCard` is an `<article>` whose title is a `<p>`, so the landmark is unnamed. `aria-labelledby` on the title, or a plain `<div>`.
- `packages/ui/src/components/FeatureGrid.tsx:46-50` — `getBoundingClientRect()` on every `pointermove`, then two custom-property writes: a read-after-write cycle at pointer frequency. Cache the rect on `pointerenter`.
- `packages/ui/src/components/FeatureGrid.tsx:1` — `"use client"` also client-ifies `FeatureGrid`, which is a `<div>` with three literal classes; only `FeatureCell` needs the handler. `Bento.tsx` avoids this.
- `packages/ui/src/motion/ScrollSettle.tsx:94` — `perspective: 1500` as an inline literal, where the theme deliberately clears `--perspective-*`.
- `packages/ui/src/components/Compare.tsx:62` — `var(--gp-border-strong)` inside an arbitrary class value for a background; a two-line `@utility` beside `spotlight` would put it where the rest of the hand-written CSS lives. Same file `:19` types a `56px` grid column.
- `packages/ui/src/components/Stepper.tsx:12-13` and `ScrollSettle.tsx` — two docstring claims a shade stronger than the code: «progress is 1 from the first paint» (an effect sets it, so the server paint is unfilled) and no sentence about a full-motion visitor below the fold seeing flat and then tilting back to 18°.

**`apps/landing`**

- `apps/landing/components/blocks/footer.tsx:17,30` — heading order skips `h3`: the last `h2` in `<main>` is `cta.tsx:15` and the footer opens at `h4`.
- `apps/landing/components/blocks/nav.tsx:20-24` — `aria-current` is never cleared: scrolling back into the hero or past `#faq` leaves the underline on a section the reader has left.
- `apps/landing/components/visuals/board.tsx:25` — `hidden grid` in one class list relies on Tailwind emitting `.hidden` after `.grid`; it holds today by emission order, not intent. Spell it `hidden md:grid`.
- `apps/landing/components/blocks/hero.tsx:19,21`, `cta.tsx:9,16,18`, `app/og/page.tsx:13` — three unguarded `indexOf` splits on `title.indexOf(titleAccent)`, unlike `accent-text.tsx`. Nothing asserts `title.includes(titleAccent)` for any of the eleven pairs; a three-line content test retires the class.
- Spec §9.4 lists a `405` assertion in `tests/pilot-route.test.ts`; it is not there. Next auto-405s an unexported method, so the behaviour is right and only the pin is missing.
- Spec §9.4 lists «no `aria-disabled`» for `tests/landing-render.test.tsx`; that assertion is absent.
- Spec §9.3 says the five delivery variables are declared on the landing's `build`/`dev`/**`test`** turbo tasks; `turbo.json` omits them from `test`. Harmless (`vi.stubEnv`), but stated and undone.
- `apps/landing/app/api/pilot/deliver.ts:30` — `referer` is the one untrusted string with no cap and no cleaning; Telegram rejects a `sendMessage` over 4096 characters, so a long Referer turns a good delivery into a 502. `cleanField(referer, 300)`.
- `apps/landing/app/api/pilot/deliver.ts:63` — logs `String(r.reason)`; the reasons the module constructs are safe, but a transport-level rejection is whatever the runtime threw and the request URL embeds `TELEGRAM_BOT_TOKEN`. Log a channel tag and a status instead.
- `apps/landing/tests/landing-content.test.ts:29-34` — the forbidden-words list checks plain substrings for two inflections of «клієнт» only, so «клієнта», «клієнтом», «клієнтський» pass; and «кеп» / «тов » will eventually false-positive inside an unrelated word. Use the word-aware pattern the «оплат» check already demonstrates.
- Same file — «польова вебпрограма» is pinned as an exact phrase, so «польова веб-програма» would not be caught.
- Same file `:42` — «електромонтаж» is checked against `copy` only, while every other forbidden term is checked against `everything`.
- `apps/landing/components/blocks/footer.tsx:21` — `"mailto"` is an untyped sentinel compared by string; a typo in the content renders a relative link to `/mailTo` with no error.
- `apps/landing/content/landing-metadata.ts` — no canonical and no robots directive; `metadataBase` alone emits neither.
- `apps/landing/app/layout.tsx:27-33` — `metadataBase` trusts `x-forwarded-host`/`host`; a malformed value makes `new URL()` throw inside `generateMetadata` and 500s the page. Validate or fall back.
- `apps/landing/qa/landing.mjs:25,108` — `server.kill()` signals `pnpm`, not the `next start` it spawned; on a crash path the server can survive on 3111 and fail the next run. `detached` + `process.kill(-pid)`, or exec `next` directly.
- `apps/landing/qa/landing.mjs:26` — a fixed 4s sleep stands in for a readiness probe; on a cold machine the first `page.goto` races the server.
- `apps/landing/components/blocks/pilot-form.tsx:43` — a whitespace-only name focuses the wrong field: the branch reads the raw value while `validatePilotFields` trims.
- `apps/landing/components/blocks/pilot-form.tsx:55` — `form.reset()` resets the DOM but not `fields` or `role`; the Radix `Select` keeps the previous role and a second submission carries stale `fields` into the mailto fallback.
- `apps/landing/app/globals.css:1-10,26-29` — two stale comments: the file is described as «either a source exclusion or a font declaration» above a 52-line `@layer components` block, and the «display face … ~40 KB» paragraph describes a face Daylight removed.
- The QA harness proves layout, not semantics. Everything in the review's accessibility cluster would have been caught by an axe pass at two widths and by no number of screenshots; `@axe-core/puppeteer` beside the overflow check in `qa/landing.mjs` is the cheap guard.
- One helper for the two raw forwarded-header reads (`app/api/pilot/route.ts:21`, `app/layout.tsx:27-31`): `rate-limit.ts`'s comment calls the header attacker-controlled while `route.ts` takes element `[0]` from it, and the two statements should not disagree inside one feature.

**Documents and configuration**

- `docs/design/01-tokens.md:318` (GENERATED — the text lives in `packages/tokens/src/tokens.json`) and `DESIGN.md:280` put the pilot form on `card` (12px); `blocks/pilot-form.tsx:76` ships `rounded-surface` (14px).
- `DESIGN.md:281` scopes `section` (16px) to «the closing CTA card» only; `visuals/channel-app.tsx` and `visuals/ui-capture.tsx` use it for the phone bezels, correctly.
- `DESIGN.md:159-161,258-262` — the `shadow-float` «reserved for» list omits `visuals/ui-window.tsx`, `visuals/channel-web.tsx` and `visuals/ui-capture.tsx`, which all carry it legitimately. Add them or soften «reserved for» to «carried by».
- `DESIGN.md:115-126` — the `feature-cell` frontmatter entry attributes a border and radius that live on the `FeatureGrid` container, not the cell; `compare-card` omits the border every `CompareCard` carries. The prose at `:314-315` gets both right.
- `docs/design/2026-08-19-design-system-rewrite-plan.md:225,344` — D3 and the font table still name Source Serif 4 as the display face and Inter for all UI, while a sibling section got a dated 2026-09-05 Update. Same file `:507,677,696` still say «fifteen» 130 lines below an Update that says sixteen. Decide whether the plan is a living companion or a historical record, and mark it either way.
- `design-references/visual-directions/README.md:1,9` — «three visual directions» and a lock table (Carbon, Lime, Manrope, Inter) that Direction 04 does not use. Needs «четыре» and a dated Russian note that Direction 04 is outside the lock.
- Same file `:111-113` — the superseded note cites `design-references/contest-2026-09/` for a README that is at `contest-2026-09/daylight/README.md`.
- `docs/design/03-ui-references.md:4` — the file header still scopes the document to `apps/app/app/(dash)/**` though it now carries a section for `apps/landing/**`, and that section introduces a second per-section Applies-to block inside a file that already has a file-level one.
- Same file `:137-158` — the 21st.dev section gives the structure-only rule but never names the licence, in the one file whose own standard (`:86`) is «name the licence».
- `infra/README-staging.md:467-470` — five variable names in a flat sequence with no statement of which pair with which, and Production only, while the sibling §4.3 is «Production AND Preview, every one». A Preview deployment of the landing would 503 with no note saying why.
