...
- **P3 — `design-contract.test.tsx`'s `it()` title still says «ships the approved evidence-journey direction»** (`apps/landing/tests/design-contract.test.tsx:8`) though the contract id it asserts is `user-approved-daylight-2026-09-05`. The assertion is correct; only the test's own name is stale.
