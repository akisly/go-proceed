...
const BASE_FILE = process.env.PROSPECTS_BASE_FILE || 'prospects_unified_v7_2026-08-24.json';
const UNIFIED_FILE = process.env.PROSPECTS_UNIFIED_FILE || 'prospects_unified_v8_2026-08-24.json';
const base = JSON.parse(fs.readFileSync(path.join(OUT_DIR, BASE_FILE), 'utf8'));
const unified = JSON.parse(fs.readFileSync(path.join(OUT_DIR, UNIFIED_FILE), 'utf8'));
const overlaps = JSON.parse(fs.readFileSync(path.join(OUT_DIR, `prozorro_wave${WAVE}_validated_overlaps_${DATE_TAG}.json`), 'utf8'));
const summary = JSON.parse(fs.readFileSync(path.join(OUT_DIR, `wave${WAVE}_normalization_summary_${DATE_TAG}.json`), 'utf8'));
...
const duplicateGroups = (rows, keyFn) => {
  ...
  return [...grouped.entries()].filter(([,items])=>items.length>1).map(([key,items])=>({
    key,
    rows:items.map(x=>({lead_id:x.lead_id,company_name:x.company_name,edrpou:x.edrpou,wave:x.wave})),
  }));
};
...
const newRows=unified.filter(x=>String(x.lead_id || '').startsWith(`W${WAVE}-`));
...
const waveSegmentCounts=newRows.reduce((a,x)=>((a[x.segment]=(a[x.segment] || 0)+1),a),{});
...
const report={
  ok:
    ...
    duplicateEdrpou.length === 0 &&
    missingLeadIds.length === 0 &&
    emptyCompanies.length === 0 &&
    invalidTenderIds.length === 0 &&
    unified.length === base.length + summary.unique_new_supplier_leads &&
    newRows.length === summary.unique_new_supplier_leads &&
    previous51.length === 51 &&
    previous51Missing.length === 0 &&
    previous51.every(x=>x.evidence_confidence === 'high'),
  counts:{
    base_rows:base.length,
    unified_rows:unified.length,
    expected_unified_rows:base.length + summary.unique_new_supplier_leads,
    wave_rows:newRows.length,
    ...
    previous_51_rows:previous51.length,
    previous_51_high_confidence:previous51.filter(x=>x.evidence_confidence === 'high').length,
  },
  ...
  waveSegmentCounts:Object.fromEntries(Object.entries(waveSegmentCounts).sort((a,b)=>b[1]-a[1] || a[0].localeCompare(b[0],'uk'))),
  previous51VerificationCounts:previous51.reduce((a,x)=>((a[x.verification_status || '—']=(a[x.verification_status || '—'] || 0)+1),a),{}),
};
fs.writeFileSync(path.join(OUT_DIR,`wave${WAVE}_v${WAVE + 1}_validation_${DATE_TAG}.json`),JSON.stringify(report,null,2));
