...
const unified = JSON.parse(fs.readFileSync(path.join(OUT_DIR, 'prospects_unified_v7_2026-08-24.json'), 'utf8'));
const overlaps = JSON.parse(fs.readFileSync(path.join(OUT_DIR, 'prozorro_wave6_validated_overlaps_2026-08-24.json'), 'utf8'));
const summary = JSON.parse(fs.readFileSync(path.join(OUT_DIR, 'wave6_normalization_summary_2026-08-24.json'), 'utf8'));
...
const newRows=unified.filter(x=>String(x.lead_id || '').startsWith('W6-'));
...
const wave6SegmentCounts=newRows.reduce((a,x)=>((a[x.segment]=(a[x.segment] || 0)+1),a),{});
...
const report={
  ...
  wave6SegmentCounts:Object.fromEntries(Object.entries(wave6SegmentCounts).sort((a,b)=>b[1]-a[1] || a[0].localeCompare(b[0],'uk'))),
  previous51VerificationCounts:previous51.reduce((a,x)=>((a[x.verification_status || '—']=(a[x.verification_status || '—'] || 0)+1),a),{}),
};
fs.writeFileSync(path.join(OUT_DIR,'wave6_v7_validation_2026-08-24.json'),JSON.stringify(report,null,2));
