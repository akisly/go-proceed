...
for (const row of readJson('prospects_unified_v6_2026-08-24.json')) {
...
async function fetchJson(url, options = {}, attempt = 1) {
  try {
    const response = await fetch(url, {
      ...
      headers:{ accept:'application/json', 'user-agent':'GoProceed-prospect-research/1.3', ...(options.headers || {}) },
    });
    if (!response.ok) {
...
fs.writeFileSync(path.join(OUT_DIR,'prozorro_wave6_search_hits_2026-08-24.json'),JSON.stringify(searchHits,null,2));
fs.writeFileSync(path.join(OUT_DIR,'prozorro_wave6_tender_records_raw_2026-08-24.json'),JSON.stringify(rawRecords,null,2));
fs.writeFileSync(path.join(OUT_DIR,'prozorro_wave6_tender_records_cleaned_2026-08-24.json'),JSON.stringify(records,null,2));
fs.writeFileSync(path.join(OUT_DIR,'prozorro_wave6_excluded_records_2026-08-24.json'),JSON.stringify(excludedRecords,null,2));
fs.writeFileSync(path.join(OUT_DIR,'prozorro_wave6_supplier_leads_2026-08-24.json'),JSON.stringify(supplierLeads,null,2));
fs.writeFileSync(path.join(OUT_DIR,'prozorro_wave6_errors_2026-08-24.json'),JSON.stringify(errors,null,2));
fs.writeFileSync(path.join(OUT_DIR,'prozorro_wave6_summary_2026-08-24.json'),JSON.stringify(summary,null,2));
