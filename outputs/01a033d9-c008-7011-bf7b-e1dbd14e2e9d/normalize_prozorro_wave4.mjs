...
const leads = JSON.parse(fs.readFileSync(path.join(OUT_DIR,'prozorro_wave4_supplier_leads_cleaned_2026-08-24.json'),'utf8'));
const records = JSON.parse(fs.readFileSync(path.join(OUT_DIR,'prozorro_wave4_tender_records_cleaned_2026-08-24.json'),'utf8'));

const browserVerifiedWebsites = new Map([
  ['40858527', { company_url:'https://www.energo.ua/uk/contractors/40858527', website_status:'browser_verified_industry_profile_2026-08-24' }],
  ['37131764', { company_url:'https://opendatabot.ua/c/37131764', website_status:'browser_verified_registry_profile_2026-08-24' }],
  ['38306994', { company_url:'https://gversbud.com.ua/', domain:'gversbud.com.ua', website_status:'supplier_provided_domain_present_2026-08-24' }],
]);
...
// When the incoming supplier has an EDRPOU, a different explicit EDRPOU in the
// base is authoritative evidence that similarly named entities are distinct.
// Domain/name fallbacks are therefore limited to historical rows without EDRPOU.
...
const highHidden = /покрів|гідроізоляц|фасад|підлог|бетон|пал|фундамент|металоконструкц|вогнезахист|теплоізоляц|чист.*приміщ|басейн|газов.*мереж|пожежогасін|каналіза|підстанц/i;

const filtered=leads.filter(x=>!isPlaceholder(x)); const overlaps=[]; const uniqueNew=[];
for(const raw of filtered){
  const rows=recordsByEdrpou.get(String(raw.edrpou))||[]; const latest=latestRecord(rows); const normalized={...raw,...(browserVerifiedWebsites.get(String(raw.edrpou))||{}),company_type:isSole(raw)?'sole_proprietor':'legal_entity',live_project_count:liveCount(rows),latest_award_date:latest?.award_date||raw.latest_award_date,latest_delivery_end_date:latest?.delivery_end_date||raw.latest_delivery_end_date,latest_tender_id:latest?.tender_id||raw.latest_tender_id,latest_tender_url:latest?.tender_url||raw.latest_tender_url,latest_tender_title:latest?.tender_title||raw.tender_titles?.[0]||''};
...
const wave4Rows=uniqueNew.map((lead,index)=>{
  const product=supported.get(lead.primary_segment); const sole=lead.company_type==='sole_proprietor'; const coverage=product?.coverage||(/СКС|BMS|слаботоч/i.test(lead.primary_segment)?2:1); const workflow=5; const hidden=highHidden.test(lead.primary_segment)?5:4; const scale=scaleSignal(lead.total_award_value_uah); const docs=5; const buyer=sole?3:5; const reachability=lead.domain?5:3; const lane=product?'Pilot now':'Expansion discovery'; const pilot=pilotScore({coverage,workflow,hidden,scale,docs,buyer,reachability}); const expansion=expansionScore({workflow,hidden,scale,docs,buyer,reachability}); const fitPriority=lane==='Pilot now'&&lead.intent_priority==='I1'?'P1':lane==='Pilot now'&&lead.intent_priority==='I2'?'P2':lane==='Expansion discovery'&&lead.intent_priority==='I1'?'D1':lane==='Expansion discovery'&&lead.intent_priority==='I2'?'D2':lane==='Pilot now'?'P3':'D3';
  return {lead_id:`W4-${String(index+1).padStart(3,'0')}`,wave:'Prozorro expansion — 24.08',company_name:lead.company_name,domain:lead.domain||'',segment:lead.primary_segment,account_type:sole?'sole_proprietor_contractor':'tender_proven_contractor',route:product?.route||'Discovery розширення — 2 тижні',current_requirement_coverage:coverage,workflow_fit:workflow,hidden_intensity:hidden,scale_signal:scale,docs_signal:docs,buyer_fit:buyer,reachability,fit_evidence:`${lead.tender_count} нових підтверджених перемог/договорів у Prozorro на ${Math.round(lead.total_award_value_uah).toLocaleString('uk-UA')} грн; ${lead.live_project_count} зі строком виконання не раніше 24.08.2026.`,pilot_trigger:`${lead.latest_tender_title}${lead.latest_delivery_end_date?`; строк до ${lead.latest_delivery_end_date.slice(0,10)}`:''}. Попросити один знеособлений пакет здачі за цим workflow.`,target_role:sole?'Власник / виконавець; відповідальний за договір':'Власник / директор; керівник ПТО; відповідальний за договори',source_url:lead.latest_tender_url,source_access:'official_prozorro_api',evidence_confidence:'high',verification_status:'official_prozorro_award_2024_2026',research_date:'2026-08-24',contact_status:'Не контактували',next_action:product?`Персоналізований free-pilot outreach по ${lead.latest_tender_id}; попросити 1 пакет Н.14/Н.15`:`Workflow-audit outreach по ${lead.latest_tender_id}; попросити 1 пакет здачі та вимоги проєкту`,edrpou:lead.edrpou,owner:'',lane,pilot_now_score:pilot,expansion_score:expansion,priority:fitPriority,intent_score:lead.intent_score,intent_priority:lead.intent_priority,tender_count:lead.tender_count,live_project_count:lead.live_project_count,total_award_value_uah:lead.total_award_value_uah,latest_tender_id:lead.latest_tender_id,latest_tender_url:lead.latest_tender_url,latest_tender_title:lead.latest_tender_title,latest_award_date:lead.latest_award_date,latest_delivery_end_date:lead.latest_delivery_end_date,tender_segments:lead.segments.join('; '),company_url:lead.company_url||'',website_status:lead.website_status||(lead.domain?'supplier_provided_url':'not_found_in_top_search')};
});
const unified=[...enrichedBase,...wave4Rows];
const summary={generated_at:'2026-08-24T23:59:59+03:00',input_supplier_leads:leads.length,invalid_or_redacted_removed:leads.length-filtered.length,valid_supplier_leads:filtered.length,overlaps_with_base_815:overlaps.length,unique_new_supplier_leads:wave4Rows.length,unified_total:unified.length,company_type_counts:wave4Rows.reduce((a,x)=>((a[x.account_type]=(a[x.account_type]||0)+1),a),{}),intent_priority_counts:wave4Rows.reduce((a,x)=>((a[x.intent_priority]=(a[x.intent_priority]||0)+1),a),{}),lane_counts:unified.reduce((a,x)=>((a[x.lane]=(a[x.lane]||0)+1),a),{}),high_confidence_count:unified.filter(x=>x.evidence_confidence==='high').length,tender_signal_count:unified.filter(x=>Number(x.tender_count||0)>0).length,live_project_signal_count:unified.filter(x=>Number(x.live_project_count||0)>0).length};
fs.writeFileSync(path.join(OUT_DIR,'prozorro_wave4_validated_leads_2026-08-24.json'),JSON.stringify(uniqueNew,null,2));
fs.writeFileSync(path.join(OUT_DIR,'prozorro_wave4_validated_overlaps_2026-08-24.json'),JSON.stringify(overlaps,null,2));
fs.writeFileSync(path.join(OUT_DIR,'prospects_unified_v5_2026-08-24.json'),JSON.stringify(unified,null,2));
fs.writeFileSync(path.join(OUT_DIR,'wave4_normalization_summary_2026-08-24.json'),JSON.stringify(summary,null,2));
console.log(JSON.stringify(summary,null,2)); console.log('Top new leads:'); console.log(wave4Rows.slice(0,30).map(x=>({id:x.lead_id,company:x.company_name,edrpou:x.edrpou,segment:x.segment,lane:x.lane,intent:x.intent_score,priority:x.intent_priority,tenders:x.tender_count,live:x.live_project_count,value:x.total_award_value_uah,domain:x.domain})));
