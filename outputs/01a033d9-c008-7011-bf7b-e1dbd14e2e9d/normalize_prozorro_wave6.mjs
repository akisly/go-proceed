...
const base = JSON.parse(fs.readFileSync(path.join(OUT_DIR, 'prospects_unified_v6_2026-08-24.json'), 'utf8'));
const leads = JSON.parse(fs.readFileSync(path.join(OUT_DIR, 'prozorro_wave6_supplier_leads_final_2026-08-24.json'), 'utf8'));
const records = JSON.parse(fs.readFileSync(path.join(OUT_DIR, 'prozorro_wave6_tender_records_cleaned_final_2026-08-24.json'), 'utf8'));

const browserVerifiedWebsites = new Map([
  ['34994941', {
    domain:'ukrnadra.com',
    company_url:'https://www.ukrnadra.com/',
    website_status:'browser_verified_official_company_site_2026-08-24',
  }],
  ['32290523', {
    domain:'ankor-group.com',
    company_url:'https://www.ankor-group.com/',
    website_status:'browser_verified_official_company_site_2026-08-24',
  }],
  ['37528539', {
    domain:'specmontazinzhiniring.com.ua',
    company_url:'http://specmontazinzhiniring.com.ua/',
    website_status:'browser_verified_official_company_site_2026-08-24',
  }],
  ['39612538', {
    domain:'trafficmg.com.ua',
    company_url:'https://trafficmg.com.ua/',
    website_status:'browser_verified_official_company_site_2026-08-24',
  }],
  ['40538615', {
    company_url:'https://opendatabot.ua/c/40538615',
    website_status:'browser_verified_registry_profile_no_company_site_found_2026-08-24',
  }],
  ['45747041', {
    company_url:'https://opendatabot.ua/c/45747041',
    website_status:'browser_verified_registry_profile_no_company_site_found_2026-08-24',
  }],
]);
...
function isPublicNonContractor(lead) {
  return /державн.*пожежно.?рятувальн.*загін|аварійно.?рятувальн.*загін|військова частина|головне управління.*дснс|територіальне управління|управління поліції охорони|комунальн.*служб.*автомобільн.*доріг/iu.test(lead.company_name || '');
}
...
const supported = new Map([
  ['Світлофори / дорожня автоматика',{route:'Пілот зараз — Н.15 subset',coverage:4}],
  ['Релейний захист / автоматика',{route:'Пілот зараз — Н.15 subset',coverage:4}],
  ['Повітряні лінії електропередачі',{route:'Пілот зараз — Н.15 subset',coverage:4}],
  ['Кабельні лінії електропередачі',{route:'Пілот зараз — Н.15 subset',coverage:4}],
  ['Залізнична електрифікація',{route:'Пілот зараз — Н.15 subset',coverage:4}],
  ['Зарядна інфраструктура',{route:'Пілот зараз — Н.15 subset',coverage:4}],
  ['Комерційний облік електроенергії',{route:'Пілот зараз — Н.15 subset',coverage:4}],
  ['Вузли обліку тепла',{route:'Пілот зараз — Н.14 subset',coverage:4}],
]);
const highHidden = /демонтаж|підпір|габіон|схил|пал|фундамент|гідротех|берего|днопоглиб|порт|тунел|підзем|аеродром|світлофор|автоматик|релейн|лінії електропередач|кабельн.*ліні|залізнич|свердлов|зрошен|меліорац|відход|полігон|модульн|підлог|резервуар|електрохім|когенерац|зарядн|облік|осад|зерно|елеватор|теплиц|пилогаз|спортивн|шумозах|мост/iu;
...
const wave6Rows = uniqueNew.map((lead,index) => {
  const product=supported.get(lead.primary_segment);
  ...
  const docs=5;
  const buyer=publicSignal ? 1 : sole ? 3 : 5;
  const reachability=lead.domain ? 5 : 3;
  const lane=publicSignal ? 'Requalify' : product ? 'Pilot now' : 'Expansion discovery';
  ...
  const fitPriority=publicSignal ? (lead.intent_priority === 'I1' ? 'R1' : 'R2') :
  ...
  return {
    lead_id:'W6-' + String(index + 1).padStart(3,'0'),
    wave:'Prozorro vertical expansion II — 24.08',
    company_name:lead.company_name,
    domain:lead.domain || '',
    segment:lead.primary_segment,
    account_type:publicSignal ? 'public_supplier_requalify' : sole ? 'sole_proprietor_contractor' : 'tender_proven_contractor',
    route:publicSignal ? 'Перевірити комерційність підрядної діяльності' : product?.route || 'Discovery розширення — 2 тижні',
    ...
    buyer_fit:buyer,
    reachability,
    fit_evidence:lead.tender_count + ' нових підтверджених перемог/договорів у Prozorro на ' + Math.round(lead.total_award_value_uah).toLocaleString('uk-UA') + ' грн; ' + lead.live_project_count + ' зі строком виконання не раніше 24.08.2026.',
    ...
    target_role:publicSignal ? 'Керівник підрозділу; перевірити право на комерційні роботи' : sole ? 'Власник / виконавець; відповідальний за договір' : 'Власник / директор; керівник ПТО; відповідальний за договори',
    ...
    next_action:publicSignal ? 'Кваліфікувати тип постачальника за ' + lead.latest_tender_id + ' до outreach' : product ? 'Персоналізований free-pilot outreach по ' + lead.latest_tender_id + '; попросити 1 пакет Н.14/Н.15' : 'Workflow-audit outreach по ' + lead.latest_tender_id + '; попросити 1 пакет здачі та вимоги проєкту',
    edrpou:lead.edrpou,
    owner:'',
    lane,
...
const unified=[...enrichedBase,...wave6Rows];
const summary={
  ...
  overlaps_with_base_1688:overlaps.length,
  unique_new_supplier_leads:wave6Rows.length,
  unified_total:unified.length,
  public_supplier_requalify:wave6Rows.filter(x=>x.account_type === 'public_supplier_requalify').length,
  company_type_counts:wave6Rows.reduce((a,x)=>((a[x.account_type]=(a[x.account_type] || 0)+1),a),{}),
  intent_priority_counts:wave6Rows.reduce((a,x)=>((a[x.intent_priority]=(a[x.intent_priority] || 0)+1),a),{}),
...
fs.writeFileSync(path.join(OUT_DIR,'prozorro_wave6_validated_leads_2026-08-24.json'),JSON.stringify(uniqueNew,null,2));
fs.writeFileSync(path.join(OUT_DIR,'prozorro_wave6_validated_overlaps_2026-08-24.json'),JSON.stringify(overlaps,null,2));
fs.writeFileSync(path.join(OUT_DIR,'prospects_unified_v7_2026-08-24.json'),JSON.stringify(unified,null,2));
fs.writeFileSync(path.join(OUT_DIR,'wave6_normalization_summary_2026-08-24.json'),JSON.stringify(summary,null,2));
console.log(JSON.stringify(summary,null,2));
console.log('Top new leads:');
console.log(wave6Rows.slice(0,35).map(x=>({id:x.lead_id,company:x.company_name,edrpou:x.edrpou,segment:x.segment,lane:x.lane,intent:x.intent_score,priority:x.intent_priority,tenders:x.tender_count,live:x.live_project_count,value:x.total_award_value_uah,domain:x.domain})));
