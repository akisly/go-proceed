...
Nothing else belongs here. `.gitignore` ignores everything under `outputs/` except this file, and `pnpm validate:canonical-docs` refuses any other tracked file here, any tracked file it cannot read (spreadsheets, archives, PDFs, documents, databases, any other binary but images and fonts) unless approved, and any tracked file that carries a ProZorro `contactPoint` ([BL-081](../docs/BACKLOG.md#bl-081), [DEV-031](../docs/tasks/DEV-031-outputs-guards.md)).
